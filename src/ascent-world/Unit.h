...
//	std::map<uint32,ProcTriggerSpellOnSpellList> m_procSpellonSpell; //index is namehash
	...
	void RemoveAurasByBGR_one_buff_from_caster(uint32 buff_index_type, const uint64 &guid);
	void RemoveAurasByBGR_one_buff_from_caster(uint32 buff_type, const uint64 &guid,uint32 skip);
	bool HasAurasOfBGR_one_buff_from_caster(uint32 buff_type, const uint64 &guid,uint32 skip);
