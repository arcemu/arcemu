...
void LogonCommHandler::Startup()
{
	...
	if(result != NULL)
	{
		do
		{
			...
			Arcemu::Shared::Util::arcemu_TOUPPER(acct);
...
void LogonCommHandler::TestConsoleLogon(string & username, string & password, uint32 requestnum)
{
	...
	Arcemu::Shared::Util::arcemu_TOUPPER(newuser);
	Arcemu::Shared::Util::arcemu_TOUPPER(newpass);
