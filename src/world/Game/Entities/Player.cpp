...
void Player::EventCannibalize(uint32 amount)
{
	...
	Messenger::SendPeriodicHealAuraLog( GetNewGUID(), this, 20577, amt, 0, false );
}
	...
	if(pMapinfo)
	{
		if(pMapinfo->HasFlag(WMI_INSTANCE_WELCOME) && GetMapMgr())
		{
			...
			if(pMapinfo->type != INSTANCE_NONRAID && !(pMapinfo->type == INSTANCE_MULTIMODE && iInstanceType >= MODE_HEROIC) && m_mapMgr->pInstance)
			{
				...
				welcome_msg += Arcemu::Shared::Util::ConvertTimeStampToDateTime((uint32)m_mapMgr->pInstance->m_expiration);
			}
...
bool CMovementCompressorThread::run()
{
	...
	while(running)
	{
		...
		m_listLock.Release();
		Arcemu::Shared::Util::Sleep(World::m_movementCompressInterval);
	}
