...
Unit * MobAI::findTarget()
{
	...
	{
		itr = itr2++;
		if( !(*itr)->IsInWorld() || !(*itr)->IsUnit() || !TO_UNIT(*itr)->IsAlive() || !TO_CREATURE(m_Self)->CanSee(TO_UNIT(*itr) ) || !( (*itr)->m_phase & m_Self->m_phase) || (*itr)->IsPlayer() && TO_PLAYER(*itr)->GetTaxiState() )
			continue;
...
void MobAI::OnAgentMelee()
{
	...
	float combatReach = calcCombatRange(target, false);
	if(distance > combatReach )
	{
		calcDestinationAndMove(target, m_Self->GetModelHalfSize() );
