...
Unit::Unit()
{
	...
	m_soulSiphon.amt = 0;
	m_soulSiphon.max = 0;
	m_modelhalfsize = 1.0f;		// worst case unit size. (Should be
								// overwritten)
...
Unit::~Unit()
{
	// start to remove badptrs, if you delete from the heap null the ptr's
	// damn!
	RemoveAllAuras();
	...
	for (std::list < struct ReflectSpellSchool * >::iterator i =
		 m_reflectSpellSchool.begin(); i != m_reflectSpellSchool.end(); i++)
		delete *i;
	m_reflectSpellSchool.clear();

	for (std::list < ExtraStrike * >::iterator itx =
		 m_extraStrikeTargets.begin(); itx != m_extraStrikeTargets.end();
		 ++itx)
	{
		ExtraStrike *es = *itx;
		LOG_ERROR
			("ExtraStrike added to Unit %u by Spell ID %u wasn't removed when removing the Aura",
			 GetGUID(), es->spell_info->Id);
		delete es;
	}
	m_extraStrikeTargets.clear();

	// delete auras which did not get added to unit yet
	for (std::map < uint32, Aura * >::iterator i = tmpAura.begin();
		 i != tmpAura.end(); i++)
		delete i->second;
	tmpAura.clear();

	for (std::list < SpellProc * >::iterator itr = m_procSpells.begin();
		 itr != m_procSpells.end(); ++itr)
		delete *itr;
	m_procSpells.clear();
...
void Unit::Update(uint32 p_time)
{
	...
	if (!IsDead())
	{
		...
		/* //if health changed since last time. Would be perfect if it would
		   work for creatures too :)
...
bool Unit::canReachWithAttack(Unit * pVictim)
{
	...
	float selfreach;
	if (IsPlayer())
		selfreach = 5.0f;		// minimum melee range, UNIT_FIELD_COMBATREACH 
								// is too small and used eg. in melee spells
	else
		selfreach = m_floatValues[UNIT_FIELD_COMBATREACH];

	float targetradius;
	// targetradius = pVictim->m_floatValues[UNIT_FIELD_BOUNDINGRADIUS];
	// //this is plain wrong. Represents i have no idea what :)
	...
	// float attackreach = (((targetradius*targetscale) + selfreach) +
	// (((selfradius*selfradius)*selfscale)+1.50f));
	...
	if (IsPlayer())
	{
		...
		// figure out how much extra distance we need to allow for based on
		// our movespeed and latency.
		if (pVictim->IsPlayer() && TO < Player * >(pVictim)->m_isMoving)
		{
			// this only applies to PvP.
			uint32 lat =
				TO < Player * >(pVictim)->GetSession()? TO <
				Player * >(pVictim)->GetSession()->GetLatency() : 0;

			// if we're over 500 get fucked anyway.. your gonna lag! and this
			// stops cheaters too
			lat = (lat > 500) ? 500 : lat;
		...
		if (TO < Player * >(this)->m_isMoving)
		{
			// this only applies to PvP.
			uint32 lat =
				TO < Player * >(this)->GetSession()? TO <
				Player * >(this)->GetSession()->GetLatency() : 0;
...
void Unit::GiveGroupXP(Unit * pVictim, Player * PlayerInGroup)
{
	...
	Player *active_player_list[MAX_GROUP_SIZE_RAID];	// since group is
														// small we can afford 
														// to do this ratehr
														// then recheck again
														// the whole active
	...
	{
		for (itr = pGroup->GetSubGroup(i)->GetGroupMembersBegin();
			 itr != pGroup->GetSubGroup(i)->GetGroupMembersEnd(); ++itr)
		{
	...
	pGroup->Unlock();
	if (active_player_count < 1)	// killer is always close to the victim.
	...
	else
	{
		...
		// I'm not sure about this formula is correct or not. Maybe some
		// brackets are wrong placed ?
		for (int i = 0; i < active_player_count; i++)
		{
			Player *plr = active_player_list[i];
			plr->
				GiveXP(float2int32
					   (((xp * plr->getLevel()) / total_level) * xp_mod),
					   pVictim->GetGUID(), true);

			active_player_list[i]->SetFlag(UNIT_FIELD_AURASTATE,
										   AURASTATE_FLAG_LASTKILLWITHHONOR);
			if (!sEventMgr.
				HasEvent(active_player_list[i],
						 EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE))
			{
				sEventMgr.AddEvent(TO_UNIT(active_player_list[i]),
								   &Unit::EventAurastateExpire,
								   (uint32) AURASTATE_FLAG_LASTKILLWITHHONOR,
								   EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE, 20000,
								   1,
								   EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
			}
			else
			{
				sEventMgr.ModifyEventTimeLeft(active_player_list[i],
											  EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE,
											  20000);
			}
			...
			{
				uint32 pet_xp = (uint32) (CalculateXpToGive(pVictim, plr->GetSummon()) * xp_mod);	// vojta: 
																									// this 
																									// isn't 
																									// blizzlike 
																									// probably 
...
uint32 Unit::HandleProc(uint32 flag, Unit * victim, SpellEntry * CastingSpell,
						bool is_triggered, uint32 dmg, uint32 abs,
						uint32 weapon_damage_type)
{
	uint32 resisted_dmg = 0;
	++m_procCounter;
	bool can_delete = !bProcInUse;	// if this is a nested proc then we should 
									// have this set to TRUE by the father
									// proc
	...
	if (m_procCounter > 40)
	{
		/* something has proceed over 10 times in a loop :/ dump the spellids
		   to the crashlog, as the crashdump will most likely be useless. */
		// BURLEX FIX ME!
		// OutputCrashLogLine("HandleProc %u SpellId %u (%s) %u", flag,
		// spellId,
		// sSpellStore.LookupString(sSpellStore.LookupEntry(spellId)->Name),
		// m_procCounter);
		return 0;
	}

	std::list < SpellProc * >::iterator itr, itr2;
	for (itr = m_procSpells.begin(); itr != m_procSpells.end();)	// Proc
																	// Trigger 
																	// Spells
																	// for
																	// Victim
	{
		...
		// Check if list item was deleted elsewhere, so here it's removed and
		// freed
		...
		if (CastingSpell != NULL)
		{
			...
			if (is_triggered
				&& !spell_proc->CanProcOnTriggered(victim, CastingSpell))
				continue;
		}
		...
		if (flag & PROC_ON_CAST_SPELL && CastingSpell
			&& !spell_proc->CheckClassMask(victim, CastingSpell))
			continue;
		...
		if (spell_proc->mOrigSpell != NULL)
			...
			origId = 0;
		SpellEntry *ospinfo = dbcSpell.LookupEntry(origId);	// no need to
															// check if exists 
															// or not since we 
															// were not able
															// to register
		...
		if (spell_proc->mProcFlags & PROC_ON_CAST_SPECIFIC_SPELL)
		{
			...
			// this is wrong, dummy is too common to be based on this, we
			// should use spellgroup or something
			if (spe->spellIconID != CastingSpell->spellIconID)
			{
				...
				if (ospinfo->School != CastingSpell->School)
					continue;
				if (CastingSpell->EffectImplicitTargetA[0] == 1 || CastingSpell->EffectImplicitTargetA[1] == 1 || CastingSpell->EffectImplicitTargetA[2] == 1)	// Prevents 
																																								// school 
																																								// based 
																																								// procs 
																																								// affecting 
		...
		if (this->IsPlayer()
			&& (spe->NameHash == SPELL_HASH_MAGTHERIDON_MELEE_TRINKET
				...
				|| spe->NameHash == SPELL_HASH_FROSTBRAND_ATTACK
				|| spellId == 16870))
		{
			float ppm = 1.0f;
			switch (spe->NameHash)
			...
			Item *mh =
				...
				GetInventoryItem(EQUIPMENT_SLOT_MAINHAND);
			Item *of =
				TO <
				Player *
				>(this)->GetItemInterface()->
				GetInventoryItem(EQUIPMENT_SLOT_OFFHAND);
			...
			if (TO < Player * >(this)->IsInFeralForm())
			{
				...
				else if (TO < Player * >(this)->GetShapeShift() == FORM_BEAR
						 || TO < Player * >(this)->GetShapeShift() ==
						 FORM_DIREBEAR)
				{
		...
		SM_FIValue(SM_FChanceOfSuccess, (int32 *) & proc_Chance,
				   ospinfo->SpellGroupType);
		if (!Rand(proc_Chance))
			continue;
		...
		// since we did not allow to remove auras like these with interrupt
		// flag we have to remove them manually.
		...
		if (spell_proc->
			DoEffect(victim, CastingSpell, flag, dmg, abs, dmg_overwrite,
					 weapon_damage_type))
			continue;
		...
		if (IsPlayer())
		{
			...
			switch (spellId)
			{
			case 32747:		// Deadly Throw Interrupt (rogue arena gloves
								// set)
				{
					if (CastingSpell == NULL)
						continue;
			...
			case 14189:		// Seal Fate
				{
					if (!this->IsPlayer() ||
						...
						CastingSpell->Id == 16953 || CastingSpell->Id == 16959)
						continue;
					if (CastingSpell->Effect[0] !=
						SPELL_EFFECT_ADD_COMBO_POINTS
						&& CastingSpell->Effect[1] !=
						SPELL_EFFECT_ADD_COMBO_POINTS
						&& CastingSpell->Effect[2] !=
						SPELL_EFFECT_ADD_COMBO_POINTS
			...
			case 37310:		// Bloodlust
				{
					if (!this->IsPlayer()
						|| TO < Player * >(this)->GetShapeShift() != FORM_CAT)
						continue;
				}
				break;
			case 16459:
				{
					...
					if (itMH != NULL)
						reqskillMH =
							GetSkillByProto(itMH->GetProto()->Class,
											itMH->GetProto()->SubClass);

					if (itOH != NULL)
						reqskillOH =
							GetSkillByProto(itOH->GetProto()->Class,
											itOH->GetProto()->SubClass);

					if (reqskillMH != SKILL_SWORDS
						&& reqskillMH != SKILL_2H_SWORDS
						&& reqskillOH != SKILL_SWORDS
						&& reqskillOH != SKILL_2H_SWORDS)
						continue;
				}
				break;
			case 12721:
			...
			case 12964:
				{
					// let's recalc chance to cast since we have a full 100
					// all time on this one
					Item *it;
					it = TO <
						Player *
						>(this)->GetItemInterface()->
					...
					if (it == NULL)
						continue;	// no weapon no joy
					// float
					// chance=float(it->GetProto()->Delay)*float(talentlevel)/600.0f;
					uint32 chance = it->GetProto()->Delay * talentlevel / 300;	// zack 
																				// this 
																				// had 
																				// a 
																				// very 
			...
			case 12654:
				{
					...
					if (CastingSpell->School != SCHOOL_FIRE)
						continue;
					SpellEntry *spellInfo = dbcSpell.LookupEntry(spellId);	// we 
																			// already 
																			// modified 
																			// this 
																			// spell 
																			...
																			// exist
					SpellDuration *sd =
						dbcSpellDuration.LookupEntryForced(spellInfo->
														   DurationIndex);
					uint32 tickcount =
						GetDuration(sd) / spellInfo->EffectAmplitude[0];
					dmg_overwrite[0] =
						ospinfo->EffectBasePoints[0] * dmg / (100 * tickcount);
				}
				break;
				// druid - Primal Fury
			case 37116:
			...
			case 31125:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// only trigger effect for specified spells
					if (CastingSpell->NameHash != SPELL_HASH_BACKSTAB &&	// backstab
						CastingSpell->NameHash != SPELL_HASH_SINISTER_STRIKE &&	// sinister 
																				// strike
			...
			case 17800:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// only trigger effect for specified spells
					if (CastingSpell->NameHash != SPELL_HASH_SHADOW_BOLT)	// shadow 
																			// bolt
						continue;
				}
				break;
			...
			case 27285:
				{
					...
					if (flag & PROC_ON_SPELL_HIT_VICTIM)
					{
						if (!CastingSpell)
							continue;
						// only trigger effect for specified spells
						if (CastingSpell->NameHash !=
							SPELL_HASH_SEED_OF_CORRUPTION)
							continue;
						// this spell builds up n time
						spell_proc->mProcCharges += dmg;
						if ((int32) spell_proc->mProcCharges >= ospinfo->EffectBasePoints[1] &&	// if 
																								// charge 
																								// built 
																								// up
							dmg < this->GetUInt32Value(UNIT_FIELD_HEALTH))	// if 
																			// this 
																			// is 
																			// not 
																			// a 
					...
					if (new_caster && new_caster->isAlive())
					{
						SpellEntry *spellInfo = dbcSpell.LookupEntry(spellId);	// we 
																				...
																				// exist
						Spell *spell =
							sSpellFactoryMgr.NewSpell(new_caster, spellInfo,
													  true, NULL);
			...
			case 18371:
				{
					...
					dmg_overwrite[0] =
						(ospinfo->EffectBasePoints[2] +
						 1) * GetMaxPower(POWER_TYPE_MANA) / 100;
				}
				break;
			...
			case 17941:
				{
					...
					if (CastingSpell->NameHash != SPELL_HASH_CORRUPTION &&	// Corruption
						CastingSpell->NameHash != SPELL_HASH_DRAIN_LIFE)	// Drain 
																			// Life
						continue;
				}
				break;
			...
			case 32391:
				{
					...
					else
					{
						if (CastingSpell->NameHash != SPELL_HASH_FIRE_SHIELD_II &&	// Corruption
							...
							CastingSpell->NameHash != SPELL_HASH_SIPHON_LIFE &&	// Siphon 
																				// Life
			...
			case 18118:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// only trigger effect for specified spells
					skilllinespell *skillability =
						objmgr.GetSpellSkill(CastingSpell->Id);
					if (!skillability)
						continue;
			...
			case 30300:
				...
				break;
				// warlock - Soul Leech - this whole spell should get
				// rewritten. Uses bad formulas, bad trigger method, spell is
				// rewritten ...
			case 30294:
				{
					if (CastingSpell == NULL)
					...
					Spell *spell =
						sSpellFactoryMgr.NewSpell(this, spellInfo, true, NULL);
					spell->SetUnitTarget(this);
					spell->Heal(amount * (ospinfo->EffectBasePoints[0] + 1) /
								100);
					delete spell;
					spell = NULL;
					continue;
			...
			case 18093:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// only trigger effect for specified spells
					if (CastingSpell->NameHash != SPELL_HASH_RAIN_OF_FIRE &&	// Rain 
																				// of 
																				// Fire
						CastingSpell->NameHash != SPELL_HASH_HELLFIRE_EFFECT &&	// Hellfire
						CastingSpell->NameHash != SPELL_HASH_SOUL_FIRE)	// Soul 
																		// Fire
						continue;
				}
				break;
			...
			case 28682:
				{
					...
					if (flag & PROC_ON_SPELL_CRIT_HIT)
					{
						...
						if (spell_proc->mProcCharges >= 3)	// whatch that
															// number cause it 
															// depends on
															// original stack
															// count !
			...
			case 27818:
				{
					if (!IsPlayer() || !dmg)
						continue;
					SpellEntry *parentproc =
						dbcSpell.LookupEntryForced(origId);
					SpellEntry *spellInfo =
						dbcSpell.LookupEntryForced(spellId);
					if (!parentproc || !spellInfo)
						continue;
					int32 val = parentproc->EffectBasePoints[0] + 1;
					Spell *spell =
						sSpellFactoryMgr.NewSpell(this, spellInfo, true, NULL);
					spell->forced_basepoints[0] = (val * dmg) / 300;	// per 
																		// tick
					SpellCastTargets targets;
			...
			case 29203:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// only trigger effect for specified spells
					if (CastingSpell->NameHash != SPELL_HASH_HEALING_WAVE)	// healing 
																			// wave
						continue;
				}
				break;
			...
			case 30165:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// only trigger effect for specified spells
					if (!(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING))	// healing 
																				// wave
						continue;
				}
				break;
			...
			case 52008:
				{
					if (CastingSpell == NULL)
						continue;
					if (!(CastingSpell->c_is_flags & SPELL_FLAG_IS_HEALING))	// healing 
																				// spell
						continue;
				}
				break;
			...
			case 51566:
				{
					if (CastingSpell == NULL)
						continue;
					if (!
						(CastingSpell->NameHash == SPELL_HASH_CHAIN_HEAL
			...
			case 25505:
				{
					if (!IsPlayer())
						continue;
					// !! The weird thing is that we need the spell that
					// triggered this enchant spell in order to output logs
					// ..we are using oldspell info too
					...
					uint32 dmg2 =
						TO < Player * >(this)->GetMainMeleeDamage(AP_owerride);
					SpellEntry *sp_for_the_logs =
						dbcSpell.LookupEntry(spellId);
					Strike(victim, MELEE, sp_for_the_logs, dmg2, 0, 0, true,
						   false);
					Strike(victim, MELEE, sp_for_the_logs, dmg2, 0, 0, true,
						   false);
			...
			case 14157:
				{
					...
					if (!
						(CastingSpell->
						 c_is_flags & SPELL_FLAG_IS_FINISHING_MOVE)
						|| victim == this)
						continue;
			...
			case 28385:
				{
					if (CastingSpell && (this == victim || !(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING)))	// no 
																													// self 
																													// casts 
																													// allowed 
																													// or 
			...
			case 15269:
				{
					...
					if (CastingSpell->School != SCHOOL_SHADOW
						|| !(CastingSpell->
							 c_is_flags & SPELL_FLAG_IS_DAMAGING))
						continue;
				}
				break;
			...
			case 23691:
				{
					if (!CastingSpell
						|| CastingSpell->NameHash != SPELL_HASH_BERSERKER_RAGE)
						continue;
				}
				break;
			...
			case 29077:
				{
					if (CastingSpell == NULL)
						continue;
					if (CastingSpell->School != SCHOOL_FIRE && CastingSpell->School != SCHOOL_FROST)	// fire 
																										// and 
																										// frost 
																										// critical's
						continue;
					dmg_overwrite[0] =
						CastingSpell->manaCost *
						(ospinfo->EffectBasePoints[0] + 1) / 100;
				}
				break;
			...
			case 33619:
				{
					...
					int power_word_id =
						HasAurasWithNameHash(SPELL_HASH_POWER_WORD__SHIELD);
					if (!power_word_id)
						power_word_id = 17;
					// make a direct strike then exit rest of handler
					int tdmg = abs * (ospinfo->EffectBasePoints[0] + 1) / 100;
					// somehow we should make this not caused any threat (to
					// be done)
					SpellNonMeleeDamageLog(victim, power_word_id, tdmg, false,
										   true);
					continue;
				} break;
			...
			case 35548:
				{
					...
					Item *it =
						...
						GetInventoryItem(EQUIPMENT_SLOT_OFFHAND);
					if (it == NULL
						|| it->GetProto()->InventoryType != INVTYPE_WEAPON)
						continue;
				}
				break;
			...
			case 54153:
				{
					...
					if (CastingSpell->Id != 53408 &&
						CastingSpell->Id != 53407 && CastingSpell->Id != 20271)
						continue;
				}
				break;
			...
			case 64850:
				{
					...
					dmg_overwrite[0] =
						(CastingSpell->
						 EffectBasePoints[IsHealingSpell(CastingSpell) - 1] +
						 1) * (ospinfo->EffectBasePoints[0] + 1) / 100;
				}
				break;
			...
			case 54180:
				{
					if (CastingSpell == NULL)
						continue;
					if (CastingSpell->Id != 53408 && CastingSpell->Id != 53407
						&& CastingSpell->Id != 20271)
						continue;
					if (!IsPlayer())
						continue;
			...
			case 43751:
				...
				break;
				// Spell Haste Trinket
				// http://www.wowhead.com/?item=28190 scarab of the infinite
				// circle
			case 33370:
				{
					if (CastingSpell == NULL)
			...
			case 39805:
				{
					if (CastingSpell == NULL)
						continue;	// this should not occur unless we made a
									// fuckup somewhere
					// trigger on lightning and chain lightning. Spell should
					// be identical , well maybe next time :P
					if (CastingSpell->NameHash == SPELL_HASH_LIGHTNING_BOLT
						|| CastingSpell->NameHash ==
			...
			case 35084:
				{
					if (CastingSpell == NULL)
						continue;
					if (!(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING))	// requires 
																				// offensive 
																				// spell. 
																				// ! 
																				// might 
			...
			case 37379:
				{
					if (!CastingSpell || CastingSpell->School != SCHOOL_SHADOW
						|| !(CastingSpell->
							 c_is_flags & SPELL_FLAG_IS_DAMAGING))
						continue;
				}
			...
			case 37378:
				{
					if (!CastingSpell || CastingSpell->School != SCHOOL_FIRE
						|| !(CastingSpell->
							 c_is_flags & SPELL_FLAG_IS_DAMAGING))
						continue;
				}
			...
			case 37237:
				{
					if (!CastingSpell
						|| CastingSpell->NameHash != SPELL_HASH_LIGHTNING_BOLT)
						continue;
				}
				break;
			...
			case 61082:
				...
				break;
				// Tier 5 Paladin setbonus - Crystalforge Battlegear or
			...
			case 43838:
				{
					if (!CastingSpell)
						continue;
					// CastingSpell->NameHash !=
					// SPELL_HASH_JUDGEMENT_OF_THE_CRUSADER &&
					if (CastingSpell->NameHash !=
			...
			case 37529:
				{
					if (!CastingSpell
						|| CastingSpell->NameHash != SPELL_HASH_OVERPOWER)
						continue;
				}
				break;
			case 37517:
				{
					if (!CastingSpell || CastingSpell->Id == 37517
						|| CastingSpell->NameHash != SPELL_HASH_REVENGE)
						continue;
				}
				break;
			case 38333:		// Ribbon of Sacrifice
				...
				// SETBONUSES END
				// http://www.wowhead.com/?item=32493 Ashtongue Talisman of
				// Shadows
			case 40480:
				{
					if (CastingSpell == NULL
			...
			case 37656:		// don't say damaging spell but EACH time
								// spell is cast there is a chance (so can be
								// healing spell)
				{
					if (CastingSpell == NULL)
						continue;
				...
				break;
				// http://www.wowhead.com/?item=32488 Ashtongue Talisman of
				// Insight
			case 40483:
				{
					if (CastingSpell == NULL)
				...
				// http://www.wowhead.com/?item=32487 Ashtongue Talisman of
				// Swiftness
			case 40487:
				{
					if (CastingSpell == NULL
				...
				// http://www.wowhead.com/?item=32485 Ashtongue Talisman of
				// Valor
			case 40459:
				{
					if (CastingSpell == NULL
						|| (CastingSpell->NameHash != SPELL_HASH_MORTAL_STRIKE
				...
				break;
			case 28804:		// Epiphany :Each spell you cast can trigger
								// an Epiphany, increasing your mana
								// regeneration by 24 for 30 sec.
				{
					if (!CastingSpell)
						continue;
				}
			...
			case 35087:
				{
					if (CastingSpell == NULL)
						continue;
					if (!(CastingSpell->c_is_flags & SPELL_FLAG_IS_HEALING))	// requires 
																				// healing 
																				// spell.
						continue;
				}
				...
				// http://www.wowhead.com/?item=32486 Ashtongue Talisman of
				// Equilibrium
			case 40452:		// Mangle has a 40% chance to grant 140
								// Strength for 8 sec
				{
					if (CastingSpell == NULL
				...
				break;
			case 40445:		// Starfire has a 25% chance to grant up to
								// 150 spell damage for 8 sec
				{
					if (CastingSpell == NULL
						|| CastingSpell->NameHash != SPELL_HASH_STARFIRE)
						continue;
				...
				break;
			case 40446:		// Rejuvenation has a 25% chance to grant up
								// to 210 healing for 8 sec
				{
					if (CastingSpell == NULL
						|| CastingSpell->NameHash != SPELL_HASH_REJUVENATION)
						continue;
				}
				break;

				// http://www.wowhead.com/?item=32490 Ashtongue Talisman of
				// Acumen
			case 40441:		// Each time your Shadow Word: Pain deals
								// damage, it has a 10% chance to grant you
								// 220 spell damage for 10 sec
				{
					if (CastingSpell == NULL
						|| CastingSpell->NameHash !=
				...
				// http://www.wowhead.com/?item=32490 Ashtongue Talisman of
				// Acumen
			case 40440:		// Each time your Renew heals, it has a 10%
								// chance to grant you 220 healing for 5 sec
				{
					if (CastingSpell == NULL
						|| CastingSpell->NameHash != SPELL_HASH_RENEW)
						continue;
				}
				break;

				// http://www.wowhead.com/?item=32492 Ashtongue Talisman of
				// Lethality
			case 37445:		// using a mana gem grants you 225 spell
								// damage for 15 sec
				{
					if (!CastingSpell
						|| CastingSpell->NameHash != SPELL_HASH_REPLENISH_MANA)
						continue;
				}
				break;
		...
		if (spellId == 17364 || spellId == 32175 || spellId == 32176)	// Stormstrike
			continue;
		if (spellId == 22858 && isInBack(victim))	// retatliation needs
													// target to be not in
													// front. Can be cast by
													// creatures too
			continue;
	...
	while (iter != m_chargeSpells.end())
	{
		...
		if (iter2->second.count)
		{
			...
			{
				// Fixes for spells that don't lose charges when dmg is
				// absorbed
				...
				{
					SpellCastTime *sd =
						dbcSpellCastTime.LookupEntryForced(CastingSpell->
														   CastingTimeIndex);
					if (!sd)
					...
					case 16188:	// Druid - Nature's Swiftness
						{
							// if(
							// CastingSpell->School!=SCHOOL_NATURE||(!sd->CastTime||sd->CastTime>10000)) 
							// continue;
					...
					case 16166:
						...
						break;
					case 14177:	// Cold blood will get removed on
									// offensive spell
						{
							if (CastingSpell == NULL
								|| CastingSpell->Id == 36554 || victim == this
								|| isFriendly(this, victim))
								continue;
						}
						break;
					case 46916:	// Bloodsurge - Slam! effect should
									// dissapear after casting Slam only
						{
							if (CastingSpell->NameHash != SPELL_HASH_SLAM)
								continue;
						}
						break;
					case 60503:	// Taste for Blood should dissapear after
									// casting Overpower
						{
	...
	m_chargeSpellsInUse = false;
	if (can_delete)				// are we the upper level of nested procs ? If 
								// yes then we can remove the lock
		bProcInUse = false;
...
void Unit::HandleProcDmgShield(uint32 flag, Unit * attacker)
{
	...
	for (i = m_damageShields.begin(); i != m_damageShields.end();)	// Deal
																	// Damage
																	// to
																	// Attacker
	{
		i2 = i++;				// we should not proc on proc.. not get here
								// again.. not needed.Better safe then sorry.
		if ((flag & (*i2).m_flags))
		{
			if (PROC_MISC & (*i2).m_flags)
			{
				...
				this->DealDamage(attacker, (*i2).m_damage, 0, 0,
								 (*i2).m_spellId);
			}
			else
			{
				SpellEntry *ability = dbcSpell.LookupEntry((*i2).m_spellId);
				this->Strike(attacker, RANGED, ability, 0, 0, (*i2).m_damage, true, true);	// can 
																							// dmg 
																							// shields 
																							// miss 
																							// at 
...
void Unit::RegeneratePower(bool isinterrupted)
{
	...
	// druids regen every tick, which is every 100ms, at one energy, as of
	// 3.0.2
	// I don't know how mana has changed exactly, but it has, will research it 
	// - optical
	...
	if (this->IsPlayer())
	{
		...
		/* 
		   ...
		   There is a problem here for druids. Druids when shapeshifted into
		   bear have 2 power with different regen timers a) Mana (which
		   regenerates while shapeshifted b) Rage

		   Mana has a regen timer of 2 seconds Rage has a regen timer of 3
		   seconds

		   I think the only viable way of fixing this is to have 2 different
		   timers to check each individual power.
										...
										float ArmorPctReduce)
{
	...
	if ((*dmg).school_type == 0)	// physical
	{
		if (this->IsPlayer())
			ArmorReduce =
				PowerCostPctMod[0] +
				((float)pVictim->GetResistance(0) *
				 (ArmorPctReduce + TO <
				  Player *
				  >(this)->
				  CalcRating(PLAYER_RATING_MODIFIER_ARMOR_PENETRATION_RATING))
				 / 100.0f);
		else
			ArmorReduce = 0.0f;
		...
		// double Reduction = double(pVictim->GetResistance(0)) /
		// double(pVictim->GetResistance(0)+400+(85*getLevel()));
		// dmg reduction formula from xinef
		double Reduction = 0;
		if (getLevel() < 60)
			Reduction =
				double (pVictim->GetResistance(0) -
						ArmorReduce) / double (pVictim->GetResistance(0) +
											   400 + (85 * getLevel()));
		else
	if (getLevel() > 59 && getLevel() < PLAYER_LEVEL_CAP)
		Reduction =
			double (pVictim->GetResistance(0) -
					ArmorReduce) / double (pVictim->GetResistance(0) -
		...
		if (Reduction > 0.75f)
			...
			Reduction = 0;
		if (Reduction)
			dmg[0].full_damage = (uint32) (dmg[0].full_damage * (1 - Reduction));	// no 
																					// multiply 
																					// by 
																					// 0
	}
	else
	{
		...
		int32 RResist =
			float2int32((pVictim->GetResistance((*dmg).school_type) +
						 ((pVictim->getLevel() >
						   getLevel())? (pVictim->getLevel() -
										 this->getLevel()) * 5 : 0)) -
						PowerCostPctMod[(*dmg).school_type]);
		if (RResist < 0)
			RResist = 0;
		...
		// NOT WOWWIKILIKE but i think it's actually to add some fullresist
		// chance from resistances
		if (!ability
			|| !(ability->Attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
		{
			float Resistchance =
				(float)pVictim->GetResistance((*dmg).school_type) /
				(float)pVictim->getLevel();
		...
		if (AverageResistance > 0)
			(*dmg).resisted_damage =
				(uint32) (((*dmg).full_damage) * AverageResistance);
		else
			(*dmg).resisted_damage = 0;
	}
}

uint32 Unit::GetSpellDidHitResult(Unit * pVictim, uint32 weapon_damage_type,
								  SpellEntry * ability)
{
	Item *it = NULL;
	...
	if (pVictim->IsPlayer())
	{
		...
		if (weapon_damage_type != RANGED && !backAttack)
		{
			// --------------------------------block
			// chance----------------------------------------------
			block = pVictim->GetFloatValue(PLAYER_BLOCK_PERCENTAGE);	// shield 
																		// check 
																		// already 
																		// done 
																		// in
			...
			{
				if (TO < Player * >(pVictim)->HasSpell(3127)
					|| TO < Player * >(pVictim)->HasSpell(18848))
				{
		...
		victim_skill =
			float2int32(vskill + TO <
						Player *
						>(pVictim)->
						CalcRating(PLAYER_RATING_MODIFIER_DEFENCE));
	}
	// --------------------------------mob defensive
	// chances-------------------------------------
	else
	{
		if (weapon_damage_type != RANGED && !backAttack)
			dodge = pVictim->GetStat(STAT_AGILITY) / 14.5f;	// what is this
															// value?
		...
		if (pVictim->IsCreature())
		{
			...
			{
				victim_skill = std::max(victim_skill, ((int32) this->getLevel() + 3) * 5);	// used 
																							// max 
																							// to 
																							// avoid 
																							// situation 
	...
	if (this->IsPlayer())
	{
		...
		case MELEE:			// melee main hand weapon
			it = disarmed ? NULL : pr->GetItemInterface()->
				GetInventoryItem(EQUIPMENT_SLOT_MAINHAND);
			hitmodifier += pr->CalcRating(PLAYER_RATING_MODIFIER_MELEE_HIT);
			self_skill =
				float2int32(pr->
							CalcRating
							(PLAYER_RATING_MODIFIER_MELEE_MAIN_HAND_SKILL));
			break;
		case OFFHAND:			// melee offhand weapon (dualwield)
			...
			self_skill =
				float2int32(pr->
							CalcRating
							(PLAYER_RATING_MODIFIER_MELEE_OFF_HAND_SKILL));
			break;
		case RANGED:			// ranged weapon
			it = disarmed ? NULL : pr->GetItemInterface()->
				GetInventoryItem(EQUIPMENT_SLOT_RANGED);
			hitmodifier += pr->CalcRating(PLAYER_RATING_MODIFIER_RANGED_HIT);
			self_skill =
				float2int32(pr->
							CalcRating(PLAYER_RATING_MODIFIER_RANGED_SKILL));
			break;
		}

		// erm. some spells don't use ranged weapon skill but are still a
		// ranged spell and use melee stats instead
		// i.e. hammer of wrath
		if (ability && ability->NameHash == SPELL_HASH_HAMMER_OF_WRATH)
		{
			it = pr->GetItemInterface()->
				GetInventoryItem(EQUIPMENT_SLOT_MAINHAND);
			hitmodifier += pr->CalcRating(PLAYER_RATING_MODIFIER_MELEE_HIT);
			self_skill =
				float2int32(pr->
		...
		if (it)
			SubClassSkill =
				GetSkillByProto(it->GetProto()->Class,
								it->GetProto()->SubClass);
		else
	...
	else
	{
		...
		if (IsCreature())
		{
			...
			if (c->GetCreatureInfo()->Rank == ELITE_WORLDBOSS)
				self_skill = std::max(self_skill, ((int32) pVictim->getLevel() + 3) * 5);	// used 
																							// max 
																							// to 
																							// avoid 
																							// situation 
	...
	// --------------------------------by victim
	// state-------------------------------------------
	if (pVictim->IsPlayer() && pVictim->GetStandState())	// every not
															// standing state
															// is >0
	{
		hitchance = 100.0f;
	...
	{
		if (pVictim->IsPlayer())
			hitchance = std::max(hitchance, 95.0f + vsk * 0.1f + hitmodifier);	// wowwiki 
																				// multiplier 
																				// - 
																				// 0.04 
																				// but 
			...
			hitchance = std::max(hitchance, 100.0f + vsk * 0.6f + hitmodifier);	// not 
																				// wowwiki 
																				// but 
																				// more 
																				// balanced
	...
	if (ability && ability->SpellGroupType)
	{
		...
		SM_FFValue(SM_FHitchance, &spell_flat_modifers,
				   ability->SpellGroupType);
		if (spell_flat_modifers != 0)
			LOG_DEBUG
				("!!!!!spell resist mod flat %f,  spell resist bonus %f, spell group %u",
	...
	uint32 roll_results[5] =
		{ SPELL_DID_HIT_MISS, SPELL_DID_HIT_DODGE, SPELL_DID_HIT_PARRY,
SPELL_DID_HIT_BLOCK, SPELL_DID_HIT_SUCCESS };
	return roll_results[r];
}

void Unit::Strike(Unit * pVictim, uint32 weapon_damage_type,
				  SpellEntry * ability, int32 add_damage, int32 pct_dmg_mod,
				  uint32 exclusive_damage, bool disable_proc,
				  bool skip_hit_check, bool force_crit)
{
	// ==========================================================================================
	// ==============================Unacceptable Cases
	// Processing===============================
	// ==========================================================================================
	if (!pVictim || !pVictim->isAlive() || !isAlive() || IsStunned()
		|| IsPacified() || IsFeared())
		return;

	if (!(ability && ability->AttributesEx & ATTRIBUTESEX_IGNORE_IN_FRONT)
		&& !isInFront(pVictim))
	{
		if (IsPlayer())
		{
			TO < Player *
				>(this)->GetSession()->OutPacket(SMSG_ATTACKSWING_BADFACING);
			return;
		}
	}
	...
	if (pVictim->IsPlayer())
	{
		...
		if (!backAttack)
		{
			...
			// uint32 pLevel = (getLevel() > PLAYER_LEVEL_CAP) ?
			// PLAYER_LEVEL_CAP : getLevel();
			...
			Item *it2 =
				...
				GetInventoryItem(EQUIPMENT_SLOT_OFFHAND);
			if (it2 != NULL
				&& it2->GetProto()->InventoryType == INVTYPE_SHIELD)
			{
	...
	else
	{
		...
		if (pVictim->IsCreature())
		{
			if (c->GetCreatureInfo()->Rank == ELITE_WORLDBOSS)
			{
				victim_skill = std::max(victim_skill, ((int32) getLevel() + 3) * 5);	// used 
																						// max 
																						// to 
																						// avoid 
																						// situation 
	...
	{
		int32 baseDefense =
			TO < Player * >(pVictim)->_GetSkillLineCurrent(SKILL_DEFENSE,
														   false);
	...
	// did my own quick research here, seems base glancing against equal level 
	// mob is about 5%
	// and goes up 5% each level. Need to check this further.
	float diffAcapped =
		victim_skill - std::min((float)self_skill, getLevel() * 5.0f);
	...
	if (pVictim->IsPlayer())
	{
		...
		{
			crit += TO < Player * >(pVictim)->res_R_crit_get();	// this could
																// be ability
																// but in that 
																// case we
																// overwrite
	...
	hitchance = 100.0f - misschance;	// base miss chances are worked out
										// further down
	...
	crit -=
		...
		Player *
		>(pVictim)->
		CalcRating(PLAYER_RATING_MODIFIER_MELEE_CRIT_RESILIENCE) : 0.0f;
	if (crit < 0)
		crit = 0.0f;
	if (this->IsPlayer())
	{
		...
		hitmodifier +=
			(weapon_damage_type ==
			 RANGED) ? plr->
			CalcRating(PLAYER_RATING_MODIFIER_RANGED_HIT) : plr->
			CalcRating(PLAYER_RATING_MODIFIER_MELEE_HIT);

		float expertise_bonus =
			plr->CalcRating(PLAYER_RATING_MODIFIER_EXPERTISE);
	...
	if (this->IsPlayer())
	{
		...
		if (!ability && it != NULL
			&& (it->GetProto()->InventoryType == INVTYPE_WEAPON
				|| it->GetProto()->InventoryType == INVTYPE_WEAPONOFFHAND))
		{
			// offhand weapon can either be a 1 hander weapon or an offhander
			// weapon
	...
	if (this->IsPlayer() && ability
		&& TO < Player * >(this)->m_finishingmovesdodge
		&& ability->c_is_flags & SPELL_FLAG_IS_FINISHING_MOVE)
		dodge = 0.0f;
	...
	case 0:
		...
		// dirty ai agro fix
		if (pVictim->IsCreature()
			&& pVictim->GetAIInterface()->getNextTarget() == NULL)
	...
	case 1:
		...
		if (this->IsPlayer() && this->getClass() == WARRIOR)
		{
			...
			TO < Player * >(this)->UpdateComboPoints();
			if (!sEventMgr.
				HasEvent(TO < Player * >(this),
						 EVENT_COMBO_POINT_CLEAR_FOR_TARGET))
				sEventMgr.AddEvent(TO < Player * >(this),
								   &Player::NullComboPoints,
								   (uint32) EVENT_COMBO_POINT_CLEAR_FOR_TARGET,
								   (uint32) 5000, (uint32) 1, (uint32) 0);
			else
				sEventMgr.ModifyEventTimeLeft(TO < Player * >(this),
											  EVENT_COMBO_POINT_CLEAR_FOR_TARGET,
											  5000, 0);
		}

		// Rune strike
		if (pVictim->IsPlayer() && pVictim->getClass() == DEATHKNIGHT)	// omg! 
																		// dirty 
																		// hack!
		...
		if (!sEventMgr.HasEvent(pVictim, EVENT_DODGE_BLOCK_FLAG_EXPIRE))
			sEventMgr.AddEvent(pVictim, &Unit::EventAurastateExpire,
							   (uint32) AURASTATE_FLAG_DODGE_BLOCK,
							   EVENT_DODGE_BLOCK_FLAG_EXPIRE, 5000, 1, 0);
		else
			sEventMgr.ModifyEventTimeLeft(pVictim,
										  EVENT_DODGE_BLOCK_FLAG_EXPIRE, 5000,
										  0);
		break;
		// --------------------------------parry-----------------------------------------------------
	case 2:
		...
		if (pVictim->IsPlayer())
		{
			...
			pVictim->SetFlag(UNIT_FIELD_AURASTATE, AURASTATE_FLAG_PARRY);	// SB@L: 
																			// Enables 
																			// spells 
																			// requiring 
																			// parry
			if (!sEventMgr.HasEvent(pVictim, EVENT_PARRY_FLAG_EXPIRE))
				sEventMgr.AddEvent(pVictim, &Unit::EventAurastateExpire,
								   (uint32) AURASTATE_FLAG_PARRY,
								   EVENT_PARRY_FLAG_EXPIRE, 5000, 1, 0);
			else
				sEventMgr.ModifyEventTimeLeft(pVictim, EVENT_PARRY_FLAG_EXPIRE,
											  5000);
			if (TO < Player * >(pVictim)->getClass() == 1 || TO < Player * >(pVictim)->getClass() == 4)	// warriors 
																										// for 
																										// 'revenge' 
																										// and 
																										// rogues 
																										...
																										// 'riposte'
			{
				pVictim->SetFlag(UNIT_FIELD_AURASTATE, AURASTATE_FLAG_DODGE_BLOCK);	// SB@L: 
																					// Enables 
																					// spells 
																					// requiring 
																					// dodge
				if (!sEventMgr.
					HasEvent(pVictim, EVENT_DODGE_BLOCK_FLAG_EXPIRE))
					sEventMgr.AddEvent(pVictim, &Unit::EventAurastateExpire,
									   (uint32) AURASTATE_FLAG_DODGE_BLOCK,
									   EVENT_DODGE_BLOCK_FLAG_EXPIRE, 5000, 1,
									   0);
				else
					sEventMgr.ModifyEventTimeLeft(pVictim,
	...
	default:
		...
		else
		{
			...
			else
			{
				...
				if (ability && ability->Id == 3018 && IsPlayer()
					&& getClass() == HUNTER)
			...
			else
			{
				if (weapon_damage_type == MELEE && ability)
					dmg.full_damage =
						CalculateDamage(this, pVictim, MELEE,
										ability->SpellGroupType, ability);
				else
					dmg.full_damage =
						CalculateDamage(this, pVictim, weapon_damage_type, 0,
										ability);
			}
			...
			// Don't really know why it was here. It should be calculated on
			// Spel::CalculateEffect. Maybe it was bugged there...
			// if(ability && ability->SpellGroupType)
			// {
			// SM_FIValue( TO_UNIT(this)->SM_FDamageBonus, &dmg.full_damage,
			// ability->SpellGroupType );
			// SM_PIValue( TO_UNIT(this)->SM_PDamageBonus, &dmg.full_damage,
			// ability->SpellGroupType );
			// } 
			// else 
			// {
			...
			dmg.full_damage +=
				float2int32(dmg.full_damage *
							pVictim->DamageTakenPctMod[dmg.school_type]);
			dmg.full_damage +=
				float2int32(dmg.full_damage *
							DamageDoneModPCT[dmg.school_type]);

			if (dmg.school_type != SCHOOL_NORMAL)
				dmg.full_damage +=
					float2int32(dmg.full_damage *
								(GetDamageDonePctMod(dmg.school_type) - 1));

			if (ability != NULL && ability->NameHash == SPELL_HASH_SHRED)
				dmg.full_damage +=
					float2int32(dmg.full_damage *
								pVictim->
								ModDamageTakenByMechPCT[MECHANIC_BLEEDING]);
			...
			if (IsPet() && !TO < Pet * >(this)->IsSummonedPet())
				dmg.full_damage =
					(dmg.full_damage <=
					 0) ? 0 : float2int32(dmg.full_damage * TO <
										  Pet * >(this)->GetHappinessDmgMod());
			...
			case 3:
				{
					...
					if (this->getClass() == MAGE || this->getClass() == PRIEST || this->getClass() == WARLOCK)	// casters 
																												// = 
																												// additional 
																												// penalty.
					{
					...
					float damage_reduction =
						(high_dmg_mod + low_dmg_mod) / 2.0f;
					if (damage_reduction > 0)
					{
						dmg.full_damage =
							float2int32(damage_reduction * dmg.full_damage);
					}
			...
			case 4:
				{
					...
					if (shield != NULL)
					{
						...
						if (shield->GetProto()->InventoryType ==
							INVTYPE_SHIELD)
						{
							float block_multiplier =
								(100.0f + TO <
								 Player * >(pVictim)->m_modblockabsorbvalue) /
								100.0f;
							if (block_multiplier < 1.0f)
								block_multiplier = 1.0f;

							blocked_damage =
								float2int32((shield->GetProto()->Block +
											 ((TO <
											   Player *
											   >(pVictim)->
											   ...
											   pVictim->
											   GetUInt32Value
											   (PLAYER_RATING_MODIFIER_BLOCK)))
											 +
											 ((pVictim->
											   GetStat(STAT_STRENGTH) / 2.0f) -
											  1.0f)) * block_multiplier);
						...
						if (blocked_damage)
						{
							CALL_SCRIPT_EVENT(pVictim, OnTargetBlocked) (this,
																		 blocked_damage);
							CALL_SCRIPT_EVENT(this, OnBlocked) (pVictim,
																blocked_damage);
							vproc |= PROC_ON_BLOCK_VICTIM;
						}
						if (pVictim->IsPlayer())	// not necessary now but
													// we'll have blocking
													// mobs in future
						{
							pVictim->SetFlag(UNIT_FIELD_AURASTATE, AURASTATE_FLAG_DODGE_BLOCK);	// SB@L: 
							...
							if (!sEventMgr.
								HasEvent(pVictim,
										 EVENT_DODGE_BLOCK_FLAG_EXPIRE))
								sEventMgr.AddEvent(pVictim,
												   &Unit::EventAurastateExpire,
												   (uint32)
												   AURASTATE_FLAG_DODGE_BLOCK,
												   EVENT_DODGE_BLOCK_FLAG_EXPIRE,
												   5000, 1, 0);
							else
								sEventMgr.ModifyEventTimeLeft(pVictim,
			...
			case 5:
				{
					...
					// sLog.outString( "DEBUG: Critical Strike! Full_damage:
					// %u" , dmg.full_damage );
					if (ability && ability->SpellGroupType)
					{
						int32 dmg_bonus_pct = 100;
						SM_FIValue(SM_PCriticalDamage, &dmg_bonus_pct,
								   ability->SpellGroupType);
						dmgbonus = dmgbonus * dmg_bonus_pct / 100;
					}

					// sLog.outString( "DEBUG: After
					// CritMeleeDamageTakenPctMod: %u" , dmg.full_damage );
					if (IsPlayer())
					{
						if (weapon_damage_type != RANGED)
						{
							dmg.full_damage +=
								dmg.full_damage * TO <
								Player * >(this)->m_modphyscritdmgPCT / 100;
						}
						if (!pVictim->IsPlayer())
							dmg.full_damage +=
								float2int32(dmg.full_damage * TO <
											Player *
											>(this)->
											IncreaseCricticalByTypePCT
											[TO_CREATURE(pVictim)->
											 GetCreatureInfo()->Type]);
						// sLog.outString( "DEBUG: After
						// IncreaseCricticalByTypePCT: %u" , dmg.full_damage
						// );
					}
					...
					if (weapon_damage_type == RANGED)
						dmg.full_damage =
							dmg.full_damage -
							float2int32(dmg.full_damage *
										CritRangedDamageTakenPctMod[dmg.
																	school_type]);
					else
						dmg.full_damage =
							dmg.full_damage -
							float2int32(dmg.full_damage *
										CritMeleeDamageTakenPctMod[dmg.
																   school_type]);

					if (pVictim->IsPlayer())
					{
						// Resilience is a special new rating which was
						// created to reduce the effects of critical hits
						...
						float dmg_reduction_pct =
							...
							>(pVictim)->
							CalcRating
							(PLAYER_RATING_MODIFIER_MELEE_CRIT_RESILIENCE) /
							100.0f;
						if (dmg_reduction_pct > 1.0f)
							dmg_reduction_pct = 1.0f;	// we cannot resist
														// more then he is
														// criticalling us,
														// there is no point
														// of the critical
						...
						dmg.full_damage =
							float2int32(dmg.full_damage -
										dmg.full_damage * dmg_reduction_pct);
						// sLog.outString( "DEBUG: After Resilience check: %u" 
						// , dmg.full_damage );
					}

					if (pVictim->IsCreature()
						&& TO <
						Creature * >(pVictim)->GetCreatureInfo()->Rank !=
						ELITE_WORLDBOSS)
					...
					if (IsPlayer())
					{
						this->SetFlag(UNIT_FIELD_AURASTATE,
									  AURASTATE_FLAG_CRITICAL);
						if (!sEventMgr.HasEvent(this, EVENT_CRIT_FLAG_EXPIRE))
							sEventMgr.AddEvent(this,
											   &Unit::EventAurastateExpire,
											   uint32(AURASTATE_FLAG_CRITICAL),
											   EVENT_CRIT_FLAG_EXPIRE, 5000, 1,
											   0);
						else
							sEventMgr.ModifyEventTimeLeft(this,
														  EVENT_CRIT_FLAG_EXPIRE,
														  5000);
					}

					CALL_SCRIPT_EVENT(pVictim, OnTargetCritHit) (this,
																 dmg.
																 full_damage);
					CALL_SCRIPT_EVENT(this, OnCritHit) (pVictim,
														dmg.full_damage);
				}
				break;
				// --------------------------------crushing
			...
			realdamage =
				dmg.full_damage - abs - dmg.resisted_damage - blocked_damage;
			if (realdamage < 0)
			{
				realdamage = 0;
				vstate = IMMUNE;
	...
	{
		dmg.full_damage =
			pVictim->DoDamageSplitTarget(dmg.full_damage, dmg.school_type,
										 true);
	...
	if (pVictim->IsCreature())
	{
		if (pVictim->GetAIInterface()
			&& (pVictim->GetAIInterface()->getAIState() == STATE_EVADE
				|| (pVictim->GetAIInterface()->GetIsSoulLinked()
					&& pVictim->GetAIInterface()->getSoullinkedWith() !=
					this)))
		{
			vstate = EVADE;
			realdamage = 0;
	...
	// the above code was remade it for reasons : damage shield needs moslty
	// same flags as handleproc + dual wield should proc too ?

	uint32 resisted_dmg;

	// damage shield must come before handleproc to not loose 1 charge : spell 
	// gets removed before last charge
	if ((realdamage > 0 || vproc & PROC_ON_BLOCK_VICTIM)
		&& weapon_damage_type != OFFHAND)
	{
		pVictim->HandleProcDmgShield(vproc, this);
		HandleProcDmgShield(aproc, pVictim);
	}

	HandleProc(aproc, pVictim, ability, disable_proc, realdamage, abs, weapon_damage_type);	// maybe 
																							// using 
																							// dmg.resisted_damage 
																							// is 
																							// better 
	...
	resisted_dmg =
		pVictim->HandleProc(vproc, this, ability, disable_proc, realdamage,
							abs, weapon_damage_type);
	...
	if (realdamage > 0 && ability == 0)
	{
		if (IsPlayer() && TO < Player * >(this)->m_onStrikeSpells.size())
		{
			...
			for (map < SpellEntry *, pair < uint32, uint32 > >::iterator itr =
				 TO < Player * >(this)->m_onStrikeSpells.begin();
				 itr != TO < Player * >(this)->m_onStrikeSpells.end(); ++itr)
			{
				if (itr->second.first)
				{
					...
					// Cast.
					cspell =
						sSpellFactoryMgr.NewSpell(this, itr->first, true,
												  NULL);
		...
		{
			map < uint32, OnHitSpell >::iterator it2 =
				TO < Player * >(this)->m_onStrikeSpellDmg.begin();
			...
			for (; it2 != TO < Player * >(this)->m_onStrikeSpellDmg.end();)
			{
				...
				SpellNonMeleeDamageLog(pVictim, itr->second.spellid, dmg2,
									   true);
			}
		}
		...
		{
			Player *owner =
				GetMapMgr()->GetPlayer((uint32) GetSummonedByGUID());
			if (owner != NULL)
				this->Energize(owner, 34433,
							   float2int32(2.5f * realdamage + 0.5f),
							   POWER_TYPE_MANA);
		}
		// ugly hack for Bloodsworm restoring hp
		if (GetUInt64Value(UNIT_FIELD_SUMMONEDBY) != 0
			&& GetUInt32Value(OBJECT_FIELD_ENTRY) == 28017)
		{
			Player *owner =
				GetMapMgr()->
				GetPlayer((uint32) GetUInt64Value(UNIT_FIELD_SUMMONEDBY));
			if (owner != NULL)
	...
	if (!ability)
	{
		...
		SendAttackerStateUpdate(this, pVictim, &dmg, realdamage, abs,
								blocked_damage, hit_status, vstate);
	}
	else
	{
		if (realdamage > 0)		// FIX ME: add log for miss,block etc for
								// ability and ranged
		{
			// here we send "dmg.resisted_damage" for "AbsorbedDamage", "0"
			// for "ResistedDamage", and "false" for "PhysicalDamage" even
			// though "School" is "SCHOOL_NORMAL" o_O
			SendSpellNonMeleeDamageLog(this, pVictim, ability->Id, realdamage,
									   static_cast < uint8 > (dmg.school_type),
									   dmg.resisted_damage, 0, false,
									   blocked_damage,
									   ((hit_status & HITSTATUS_CRICTICAL) !=
										0), true);
		}
	...
	if (this->IsPlayer() && ability)
		TO < Player * >(this)->m_casted_amount[dmg.school_type] =
			(uint32) (realdamage + abs);
	...
	if (pVictim->IsPlayer())
	{
		...
		if (!this->IsPlayer())
		{
			...
			if (Rand
				(pr->GetSkillUpChance(SKILL_DEFENSE) *
				 sWorld.getRate(RATE_SKILLCHANCE)))
			{
				pr->_AdvanceSkillLine(SKILL_DEFENSE,
									  float2int32(1.0f *
												  sWorld.
												  getRate(RATE_SKILLRATE)));
				pr->UpdateChances();
			}
		}
		else
	...
	else
	{
		if (this->IsPlayer())	// not pvp
		{
			...
			Player *pr = TO < Player * >(this);
			if (Rand
				(pr->GetSkillUpChance(SubClassSkill) *
				 sWorld.getRate(RATE_SKILLCHANCE)))
			{
				pr->_AdvanceSkillLine(SubClassSkill,
									  float2int32(1.0f *
												  sWorld.
												  getRate(RATE_SKILLRATE)));
				// pr->UpdateChances();
	...
	if (dmg.full_damage && IsPlayer() && GetPowerType() == POWER_TYPE_RAGE
		&& !ability)
	{
		float val;
		...
		if (level <= PLAYER_LEVEL_CAP)
			...
			conv =
				3.75f / (0.0091107836f * level * level + 3.225598133f * level +
						 4.2652911f);
		...
		Item *weapon =
			(TO <
			 Player *
			 >(this)->GetItemInterface())->
			GetInventoryItem(INVENTORY_SLOT_NOT_SET,
							 (weapon_damage_type ==
		...
		// float p = ( 1 + ( TO< Player* >( this )->rageFromDamageDealt /
		// 100.0f ) );
		// LOG_DEBUG( "Rd(%i) d(%i) c(%f) f(%f) s(%f) p(%f) r(%f) rage = %f",
		// realdamage, dmg.full_damage, c, f, s, p, r, val );
	...
	if (m_extrastriketargetc > 0 && !m_extrastriketarget)
	{
		...
		for (itx = m_extraStrikeTargets.begin();
			 itx != m_extraStrikeTargets.end();)
		{
			itx2 = itx++;
			ExtraStrike *ex = *itx2;

			for (set < Object * >::iterator itr = m_objectsInRange.begin();
				 itr != m_objectsInRange.end(); ++itr)
			{
				if ((*itr) == pVictim || !(*itr)->IsUnit())
					continue;

				if (CalcDistance(*itr) < 5.0f && isAttackable(this, (*itr))
					&& (*itr)->isInFront(this) && !TO_UNIT(*itr)->IsPacified())
				{
					// Sweeping Strikes hits cannot be dodged, missed or
					// parried (from wowhead)
					bool skip_hit_check2 =
						ex->spell_info->Id == 12328 ? true : false;
					// zack : should we use the spell id the registered this
					// extra strike when striking ? It would solve a few proc
					// on proc problems if so ;)
					// Strike( TO< Unit* >( *itr ), weapon_damage_type,
					// ability, add_damage, pct_dmg_mod, exclusive_damage,
					// false, skip_hit_check );
					Strike(TO < Unit * >(*itr), weapon_damage_type,
						   ex->spell_info, add_damage, pct_dmg_mod,
						   exclusive_damage, false, skip_hit_check2);
					break;
				}
			}

			// Sweeping Strikes charges are used up regardless whether there
			// is a secondary target in range or not. (from wowhead)
			if (ex->charges > 0)
			{
				ex->charges--;
				if (ex->charges <= 0)
...
void Unit::smsg_AttackStop(Unit * pVictim)
{
	...
	else
	{
		if (!IsPlayer() || getClass() == ROGUE)
		{
			...
			sEventMgr.AddEvent(this, &Unit::EventUpdateFlag,
							   EVENT_COMBAT_TIMER, 8000, 1,
							   EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
			if (pVictim->IsUnit())	// there could be damage coming from
									// objects/enviromental
				sEventMgr.AddEvent(pVictim, &Unit::EventUpdateFlag,
								   EVENT_COMBAT_TIMER, 8000, 1, 0);
		}
		else
		{
		}
...
void Unit::AddAura(Aura * aur)
{
	...
	if (!
		(isAlive()
		 || (aur->GetSpellProto()->
			 AttributesExC & CAN_PERSIST_AND_CASTED_WHILE_DEAD)))
	{
		delete aur;
		return;
	}
	...
	if (m_mapId != 530
		&& (m_mapId != 571
			|| (IsPlayer()
				&& !TO_PLAYER(this)->
				HasSpellwithNameHash(SPELL_HASH_COLD_WEATHER_FLYING))))
		// can't use flying auras in non-outlands or non-northrend (northrend
		// requires cold weather flying)
	{
		for (uint32 i = 0; i < 3; ++i)
		{
			if (aur->GetSpellProto()->EffectApplyAuraName[i] ==
				SPELL_AURA_ENABLE_FLIGHT_WITH_UNMOUNTED_SPEED
				|| aur->GetSpellProto()->EffectApplyAuraName[i] ==
	...
	if (aur->GetSpellProto()->School
		&& SchoolImmunityList[aur->GetSpellProto()->School])
	{
		delete aur;
		return;
	}
	...
	if (aur->GetSpellProto()->AttributesExE & FLAGS6_SINGLE_TARGET_AURA)
	{
		...
		{
			prev_target_guid =
				caster->GetCurrentUnitForSingleTargetAura(aur->
														  GetSpellProto());

			if (prev_target_guid
				&& prev_target_guid != aur->GetTarget()->GetGUID())
			{
				Unit *prev_target =
					this->GetMapMgr()->GetUnit(prev_target_guid);
				if (prev_target != NULL)
					prev_target->RemoveAllAuraByNameHash(aur->GetSpellProto()->
														 NameHash);
			}
		}

		// remove aura from this unit. other player/unit may have casted on
		// this target
		...
		// 4) attacker B cast on target A, and aura is not removed from target 
		// B, because caster A is now the one that casted on target B
		if (prev_target_guid
			&& prev_target_guid != aur->GetTarget()->GetGUID())
	...
	if (!aur->IsPassive())
	{
		...
		if (aur->IsPositive())
		{
			StartCheck = MAX_POSITIVE_AURAS_EXTEDED_START;	// also check
															// talents to make 
															// sure they will
															// not stack.
															// Maybe not
		...
		// Nasty check for Blood Fury debuff (spell system based on namehashes 
		// is bs anyways)
		if (aur->GetSpellProto()->always_apply == false)
		{
			...
			if (aur->GetSpellProto()->procCharges > 0)
			{
				...
				{
					SM_FIValue(ucaster->SM_FCharges, &charges,
							   aur->GetSpellProto()->SpellGroupType);
					SM_PIValue(ucaster->SM_PCharges, &charges,
							   aur->GetSpellProto()->SpellGroupType);
				}
				maxStack = charges;
			}
			...
			// check if we already have this aura by this caster -> update
			// duration
				...
				if (m_auras[x])
				{
					if (m_auras[x]->GetSpellId() == aur->GetSpellId())
					{
						if (!aur->IsPositive()
							...
							&& (m_auras[x]->GetSpellProto()->
								c_is_flags & SPELL_FLAG_IS_MAXSTACK_FOR_DEBUFF)
							== 0)
						{
							continue;
						}
						AlreadyApplied++;
						// update duration,the same aura (update the whole
						// stack whenever we cast a new one)
					...
					else if ((aur->pSpellId != m_auras[x]->GetSpellProto()->Id))	// if 
																					// this 
																					// is 
																					// a 
																					// proc 
																					...
																					// later
					{
						...
						if (info->BGR_one_buff_on_target > 0
							&& m_auras[x]->GetSpellProto()->
							BGR_one_buff_on_target & info->
							BGR_one_buff_on_target && maxStack == 0)
						{
							deleteAur =
								HasAurasOfBuffType(info->
												   BGR_one_buff_on_target,
												   aur->m_casterGuid, 0);
						}
						// Check for auras with the same name and a different
						// rank.
						else
						{
							acr =
								AuraCheck(info, m_auras[x], aur->GetCaster());
							if (acr.Error ==
			...
			if (deleteAur)
			{
				...
				// Once stacked 5 times, each application of Deadly poison
				// also causes the poison on the Rogue's other weapon to apply
				// http://www.wowhead.com/?item=43233#comments
				if (AlreadyApplied >= maxStack
					&& info->c_is_flags & SPELL_FLAG_IS_POISON)
				{
						...
						case SPELL_HASH_DEADLY_POISON:
							{
								...
								if (mh != NULL && oh != NULL)
								{
									...
									EnchantmentInstance *ench =
										mh->
										GetEnchantment(TEMP_ENCHANTMENT_SLOT);
									if (ench)
									{
										EnchantEntry *Entry =
											ench->Enchantment;
									...
									// Only apply if both weapons are
									// enchanted and enchantment is poison and 
									// enchantment type is different
									if (mh_spell && oh_spell
										&& mh_spell != oh_spell
										&& is_mh_deadly_poison !=
										is_oh_deadly_poison)
									{
										if (mh_spell != info->Id)
											caster->CastSpell(aur->GetTarget(),
															  mh_spell, true);
										else
											caster->CastSpell(aur->GetTarget(),
															  oh_spell, true);
		...
		{
			// these auras stack to infinite and with anything. Don't ask me
			// why there is no better solution for them :P
			for (uint32 x = StartCheck; x < CheckLimit; x++)
				if (!m_auras[x])
				{
					AuraSlot = static_cast < uint16 > (x);
	...
	{
		// talents just get applied always. Maybe we should check stack for
		// these as well?
		for (uint32 x = MAX_PASSIVE_AURAS_START; x < MAX_PASSIVE_AURAS_END;
			 x++)
			if (!m_auras[x])
			{
				AuraSlot = static_cast < uint16 > (x);
		...
		// else if( m_auras[x]->GetID()==aur->GetID() ) printf("OMG stacking
		// talents ?\n");
	}
	...
	if (AuraSlot == 0xFFFF)
	{
		...
		delete aur;
		/* 
		   if ( aur != NULL ) { delete [] aur; aur = NULL; } */ return;
	}

	// Zack : if all mods were resisted it means we did not apply anything and 
	// we do not need to delete this spell either
	...
	// Zack : No idea how a new aura can already have a slot. Leaving it for
	// compatibility
	...
	// We add 500ms here to allow for the last tick in DoT spells. This is a
	// dirty hack, but at least it doesn't crash like my other method.
	// - Burlex
	if (aur->GetDuration() > 0)
	{
		sEventMgr.AddEvent(aur, &Aura::Remove, EVENT_AURA_REMOVE,
						   aur->GetDuration() + 500, 1,
						   EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT |
						   EVENT_FLAG_DELETES_OBJECT);
	}
	...
	if (!aur->IsPositive() && aur->IsCombatStateAffecting())	// Creature
	{
		...
		/* if(isAlive() && CanAgroHash(aur->m_spellProto->NameHash)) //no
		   threat for hunter's mark { Unit * pCaster = aur->GetUnitCaster();
		   if(!pCaster) return;
	...
	if (aur->GetSpellProto()->
		AuraInterruptFlags & AURA_INTERRUPT_ON_INVINCIBLE)
	{
		Unit *pCaster = aur->GetUnitCaster();
		if (pCaster)
		{
			...
			pCaster->
				RemoveAllAuraByNameHash(SPELL_HASH_BLESSING_OF_PROTECTION);
		}
	}

	// If this aura can only affect one target at a time, store this target
	// GUID for future reference
	if (aur->GetSpellProto()->AttributesExE & FLAGS6_SINGLE_TARGET_AURA)
	{
		Unit *caster = aur->GetUnitCaster();
		if (caster != NULL)
			caster->SetCurrentUnitForSingleTargetAura(aur->GetSpellProto(),
													  this->GetGUID());
	}
	...
	else if (aur->GetSpellProto()->MechanicsType == MECHANIC_BLEEDING
			 && !asc_bleed++)
...
{								// this can be speed up, if we know passive
								// \pos neg
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
		if (m_auras[x] && m_auras[x]->GetSpellId() == spellId)
		{
			m_auras[x]->Remove();
			return true;		// sky: yes, only one, see bug charges/auras
								// queues
		}
	return false;
}
...
bool Unit::RemoveAurasByHeal()
{
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		if (m_auras[x])
		{
			...
			case 43093:
				{
					if (GetUInt32Value(UNIT_FIELD_HEALTH) ==
						GetUInt32Value(UNIT_FIELD_MAXHEALTH))
					{
						m_auras[x]->Remove();
						res = true;
					}
			...
			case 38772:
				{
					uint32 p =
						m_auras[x]->GetSpellProto()->EffectBasePoints[1];
					if (GetUInt32Value(UNIT_FIELD_MAXHEALTH) * p <=
						GetUInt32Value(UNIT_FIELD_HEALTH) * 100)
					{
						m_auras[x]->Remove();
						res = true;
					}
...
{
	for (uint32 x = MAX_NEGATIVE_AURAS_EXTEDED_START;
		 x < MAX_REMOVABLE_AURAS_END; x++)
	{
		if (m_auras[x])
		{
			if (m_auras[x]->GetSpellProto()->
				AttributesExC & CAN_PERSIST_AND_CASTED_WHILE_DEAD)
				continue;
			else
				m_auras[x]->Remove();
...
void Unit::RemoveAllAuraType(uint32 auratype)
{
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
		if (m_auras[x] && m_auras[x]->HasModType(auratype))
			m_auras[x]->Remove();	// remove all morph auras containing to
									// this spell (like wolf morph also gives
									// speed)
}
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
		if (m_auras[x])
		{
			...
			if (proto->BGR_one_buff_from_caster_on_self == auratype
				...
				RemoveAura(m_auras[x]->GetSpellId());	// remove all morph
														// auras containing to 
														// this spell (like
														// wolf morph also
														// gives speed)
...
void Unit::RemoveAllAurasByRequiredShapeShift(uint32 mask)
{
	...
	for (uint32 i = MAX_REMOVABLE_AURAS_START; i < MAX_REMOVABLE_AURAS_END;
		 ++i)
	{
		aura = m_auras[i];
...
Aura *Unit::FindAuraByNameHash(uint32 namehash, uint64 guid)
{
	...
	{
		aura = m_auras[x];
		if (aura != NULL && aura->GetSpellProto()->NameHash == namehash
			&& aura->m_casterGuid == guid)
			return aura;
	}
	return NULL;
}
...
Aura *Unit::FindAuraWithAuraEffect(uint32 effect, uint32 * x)
{
	...
	for (; *x < MAX_TOTAL_AURAS_END; (*x)++)
	{
		aura = m_auras[*x];
		if (aura != NULL &&
			((aura->GetSpellProto()->Effect[0] == SPELL_EFFECT_APPLY_AURA
			  && aura->GetSpellProto()->EffectApplyAuraName[0] == effect)
			 || (aura->GetSpellProto()->Effect[1] == SPELL_EFFECT_APPLY_AURA
				 && aura->GetSpellProto()->EffectApplyAuraName[1] == effect)
			 || (aura->GetSpellProto()->Effect[2] == SPELL_EFFECT_APPLY_AURA
				 && aura->GetSpellProto()->EffectApplyAuraName[2] == effect)))
			return aura;
	}
							 ...
							 int32 base_dmg, bool isdot)
{
	...
	if (!pVictim->IsPlayer() && caster->IsPlayer())
		plus_damage +=
			TO <
			Player *
			>(caster)->IncreaseDamageByType[TO_CREATURE(pVictim)->
											GetCreatureInfo()->Type];
	// ==========================================================================================
	// ==============================+Spell Damage Bonus
	// Modifications===========================
	...
	if (spellInfo->Dspell_coef_override >= 0 && !isdot)
		plus_damage =
			float2int32(plus_damage * spellInfo->Dspell_coef_override);
	else if (spellInfo->OTspell_coef_override >= 0 && isdot)
		plus_damage =
			float2int32(plus_damage * spellInfo->OTspell_coef_override);
	else
	{
		// Bonus to DD part
		...
		{
			plus_damage =
				float2int32(plus_damage * spellInfo->fixed_hotdotcoef);
			if (caster->IsPlayer())
			{
				int durmod = 0;
				SM_FIValue(caster->SM_FDur, &durmod,
		...
		else
		{
			...
			float td =
				float (GetDuration
					   (dbcSpellDuration.
						LookupEntry(spellInfo->DurationIndex)));
			if (spellInfo->NameHash == SPELL_HASH_MOONFIRE
				...
				plus_damage =
					float2int32(plus_damage *
								(1.0f -
								 ((td / 15000.0f) /
								  ((td / 15000.0f) + dmgdoneaffectperc))));
		}
	}
	...
	if (spellInfo->baseLevel > 0 && spellInfo->maxLevel > 0)
	{
		...
		float downrank2 =
			(spellInfo->maxLevel + 5.0f) / TO < Player * >(caster)->getLevel();
		if (downrank2 >= 1 || downrank2 < 0)
			downrank2 = 1.0f;
		dmgdoneaffectperc *= downrank1 * downrank2;
	}
	// ==========================================================================================
	// ==============================Bonus Adding To Main
	// Damage=================================
	// ==========================================================================================
	int32 bonus_damage = float2int32(plus_damage * dmgdoneaffectperc);

	if ((pVictim->HasAuraWithMechanics(MECHANIC_ENSNARED)
		 || pVictim->HasAuraWithMechanics(MECHANIC_DAZED))
		&& caster->IsPlayer())
		bonus_damage += TO < Player * >(caster)->m_IncreaseDmgSnaredSlowed;

	if (spellInfo->SpellGroupType)
	{
		SM_FIValue(caster->SM_FPenalty, &bonus_damage,
				   spellInfo->SpellGroupType);
		SM_FIValue(caster->SM_FDamageBonus, &bonus_damage,
				   spellInfo->SpellGroupType);

		int dmg_bonus_pct = 0;
		SM_FIValue(caster->SM_PPenalty, &dmg_bonus_pct,
				   spellInfo->SpellGroupType);
		SM_FIValue(caster->SM_PDamageBonus, &dmg_bonus_pct,
				   spellInfo->SpellGroupType);
	...
	// ------------------------------by
	// school----------------------------------------------
	float summaryPCTmod = caster->GetDamageDonePctMod(school) - 1;	// value
																	// is
																	// initialized 
																	// with 1
	...
	summaryPCTmod +=
		pVictim->ModDamageTakenByMechPCT[spellInfo->MechanicsType];
	...
	if (caster->IsPlayer())
	{
		...
		{
			int32 selfdamage =
				float2int32(((bonus_damage * summaryPCTmod) +
							 bonus_damage) * 0.1f);
	...
	int32 res =
		(int32) ((base_dmg + bonus_damage) * summaryPCTmod + bonus_damage);
	if (res < 0)
		res = 0;
...
void Unit::SendChatMessageToPlayer(uint8 type, uint32 lang, const char *msg,
								   Player * plr)
{
	size_t UnitNameLength = 0, MessageLength = 0;
	CreatureInfo *ci =
		IsCreature()? TO_CREATURE(this)->GetCreatureInfo() : NULL;
...
void Unit::SendChatMessageAlternateEntry(uint32 entry, uint8 type, uint32 lang,
										 const char *msg)
{
	size_t UnitNameLength = 0, MessageLength = 0;
	CreatureInfo *ci;
...
void Unit::CalcDamage()
{
	...
	else
	{
		...
		float bonus =
			ap_bonus * (GetBaseAttackTime(MELEE) + TO <
						Creature * >(this)->m_speedFromHaste);
		...
		// give some diversity to pet damage instead of having a 77-78 damage
		// range (as an example)
...
void Unit::SetStandState(uint8 standstate)
{
	...
	if (IsPlayer())
		TO < Player * >(this)->GetSession()->OutPacket(SMSG_STANDSTATE_UPDATE,
													   1, &standstate);
}
...
void Unit::RemoveAurasByInterruptFlag(uint32 flag)
{
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		...
		// some spells do not get removed all the time only at specific
		// intervals
		if ((a->m_spellProto->AuraInterruptFlags & flag)
			&& !(a->m_spellProto->procFlags & PROC_REMOVEONUSE))
		{
			a->Remove();
			m_auras[x] = NULL;
		}
...
bool Unit::HasAuraVisual(uint32 visualid)
{
	...
	for (uint32 x = MAX_REMOVABLE_AURAS_START; x < MAX_REMOVABLE_AURAS_END;
		 x++)
...
void Unit::UpdateSpeed()
{
	if (GetMount() == 0)
	{
		if (IsPlayer())
			m_runSpeed =
				m_base_runSpeed * (1.0f + ((float)m_speedModifier) / 100.0f);
		else
			m_runSpeed =
				m_base_runSpeed * (1.0f + ((float)m_speedModifier) / 100.0f);
	}
	else
	{
		...
		{
			m_runSpeed =
				m_base_runSpeed * (1.0f +
								   ((float)m_mountedspeedModifier) / 100.0f);
			m_runSpeed +=
				(m_speedModifier <
				 0) ? (m_base_runSpeed * ((float)m_speedModifier) /
					   100.0f) : 0;
		}
		else
		{
	...
	m_flySpeed =
		PLAYER_NORMAL_FLIGHT_SPEED * (1.0f +
									  ((float)m_flyspeedModifier) / 100.0f);

	// Limit speed due to effects such as http://www.wowhead.com/?spell=31896
...
bool Unit::HasBuff(uint32 spellid)	// cebernic:it does not check passive
									// auras & must be visible auras
{
	for (uint32 x = MAX_POSITIVE_AURAS_EXTEDED_START;
		 x < MAX_POSITIVE_AURAS_EXTEDED_END; x++)
		if (m_auras[x] && m_auras[x]->GetSpellId() == spellid)
			return true;
...
bool Unit::HasVisialPosAurasOfNameHashWithCaster(uint32 namehash,
												 Unit * caster)
{
	for (uint32 i = MAX_POSITIVE_AURAS_EXTEDED_START;
		 i < MAX_POSITIVE_AURAS_EXTEDED_END; ++i)
		if (m_auras[i] && m_auras[i]->GetSpellProto()->NameHash == namehash
			&& m_auras[i]->GetCasterGUID() == caster->GetGUID())
			return true;
...
uint8 Unit::CastSpell(Unit * Target, uint32 SpellID, uint32 forced_basepoints,
					  bool triggered)
{
	return CastSpell(Target, dbcSpell.LookupEntryForced(SpellID),
					 forced_basepoints, triggered);
}

uint8 Unit::CastSpell(Unit * Target, SpellEntry * Sp, uint32 forced_basepoints,
					  bool triggered)
{
	if (Sp == NULL)
		return SPELL_FAILED_UNKNOWN;
...
uint8 Unit::CastSpell(Unit * Target, uint32 SpellID, uint32 forced_basepoints,
					  int32 charges, bool triggered)
{
	return CastSpell(Target, dbcSpell.LookupEntryForced(SpellID),
					 forced_basepoints, charges, triggered);
}
...
void Unit::CastSpellAoF(float x, float y, float z, SpellEntry * Sp,
						bool triggered)
{
	if (Sp == NULL)
		return;
...
void Unit::RemoveAurasByBuffType(uint32 buff_type, const uint64 & guid,
								 uint32 skip)
{
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		if (m_auras[x]			// have aura
			&& (m_auras[x]->GetSpellProto()->BGR_one_buff_on_target & buff_type)	// aura 
																					// is 
																					// in 
																					// same 
																					// group
			&& m_auras[x]->GetSpellId() != skip	// make sure to not do self
												// removes in case aura will
												// stack
			&& (!sguid || (sguid && m_auras[x]->m_casterGuid == sguid))	// we
																		// either 
																		// remove 
																		// everything 
																		// or
...
bool Unit::HasAurasOfBuffType(uint32 buff_type, const uint64 & guid,
							  uint32 skip)
{
	uint64 sguid = (buff_type == SPELL_TYPE_BLESSING
					|| buff_type == SPELL_TYPE_WARRIOR_SHOUT) ? guid : 0;

	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		if (m_auras[x]
			&& m_auras[x]->GetSpellProto()->BGR_one_buff_on_target & buff_type
			&& m_auras[x]->GetSpellId() != skip)
...
AuraCheckResponse Unit::AuraCheck(SpellEntry * proto, Object * caster)
{
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		...
		{
			// we've got an aura with the same name as the one we're trying to 
			// apply
			// but first we check if it has the same effects
			aura_sp = aura->GetSpellProto();

			if ((aura_sp->Effect[0] == proto->Effect[0]
				 && (aura_sp->Effect[0] != SPELL_EFFECT_APPLY_AURA
					 || aura_sp->EffectApplyAuraName[0] ==
					 proto->EffectApplyAuraName[0]))
				&& (aura_sp->Effect[1] == proto->Effect[1]
					&& (aura_sp->Effect[1] != SPELL_EFFECT_APPLY_AURA
						|| aura_sp->EffectApplyAuraName[1] ==
						proto->EffectApplyAuraName[1]))
				&& (aura_sp->Effect[2] == proto->Effect[2]
					&& (aura_sp->Effect[2] != SPELL_EFFECT_APPLY_AURA
						|| aura_sp->EffectApplyAuraName[2] ==
						proto->EffectApplyAuraName[2])))
			{
...
AuraCheckResponse Unit::AuraCheck(SpellEntry * proto, Aura * aur,
								  Object * caster)
{
	AuraCheckResponse resp;
...
void Unit::RemoveAurasByInterruptFlagButSkip(uint32 flag, uint32 skip)
{
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		...
		if ((a->m_spellProto->AuraInterruptFlags & flag)
			...
			&& a->m_spellProto->proc_interval == 0)
		{
			// the black sheep's of society
			if (a->m_spellProto->
				AuraInterruptFlags & AURA_INTERRUPT_ON_CAST_SPELL)
			{
				...
				case 33151:
					{
						// our luck. it got triggered on smite..we do not
						// remove it just yet
						if (m_currentSpell
							&& m_currentSpell->GetProto()->NameHash ==
							SPELL_HASH_SMITE)
							continue;
				...
				case 34936:	// Backlash
					{
						...
						if (spi && spi->NameHash != SPELL_HASH_SHADOW_BOLT
							&& spi->NameHash != SPELL_HASH_INCINERATE)
							continue;
					}
					break;
				...
				case 53489:
					{
						if (m_currentSpell
							&& m_currentSpell->m_spellInfo->NameHash ==
				...
				case 16166:	// [Shaman] Elemental Mastery
					{
						SpellEntry *spi = dbcSpell.LookupEntryForced(skip);
						if (spi
							&& !(spi->School == SCHOOL_FIRE
								 || spi->School == SCHOOL_FROST
								 || spi->School == SCHOOL_NATURE))
							continue;
					}
					break;
...
bool Unit::HasAuraWithName(uint32 name)
{
	...
	{
		if (m_auras[i] != NULL
			&& m_auras[i]->GetSpellProto()->AppliesAura(name))
			return true;
	}
...
{
	for (uint32 x = MAX_NEGATIVE_AURAS_EXTEDED_START;
		 x < MAX_NEGATIVE_AURAS_EXTEDED_END; ++x)
	{
...
bool Unit::IsPoisoned()
{
	for (uint32 x = MAX_NEGATIVE_AURAS_EXTEDED_START;
		 x < MAX_NEGATIVE_AURAS_EXTEDED_END; ++x)
		if (m_auras[x]
			&& m_auras[x]->GetSpellProto()->c_is_flags & SPELL_FLAG_IS_POISON)
			return true;
...
void Unit::SendFullAuraUpdate()
{
	...
	LOG_DEBUG("Full Aura Update: GUID: " I64FMT " - Updates: %u", GetGUID(),
			  Updates);
}
...
void Unit::RemoveAurasOfSchool(uint32 School, bool Positive, bool Immune)
{
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; ++x)
		if (m_auras[x]
			...
			&& (!Immune
				&& m_auras[x]->GetSpellProto()->
				Attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
			m_auras[x]->Remove();
}
...
bool Unit::IsDazed()
{
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; ++x)
	{
		if (m_auras[x])
		{
			if (m_auras[x]->GetSpellProto()->MechanicsType ==
				MECHANIC_ENSNARED)
				return true;
			for (uint32 y = 0; y < 3; y++)
				if (m_auras[x]->GetSpellProto()->EffectMechanic[y] ==
					MECHANIC_ENSNARED)
					return true;
		}
	}
...
void Unit::UpdateVisibility()
{
	...
	{
		plr = TO < Player * >(this);
		for (Object::InRangeSet::iterator itr2 = m_objectsInRange.begin();
			 itr2 != m_objectsInRange.end();)
		{
			pObj = (*itr2);
			++itr2;
	...
	{
		for (set < Object * >::iterator it2 = GetInRangePlayerSetBegin();
			 it2 != GetInRangePlayerSetEnd(); ++it2)
		{
...
void Unit::EventHealthChangeSinceLastUpdate()
{
	...
	if (pct < 35)
	{
		...
		RemoveFlag(UNIT_FIELD_AURASTATE,
				   AURASTATE_FLAG_HEALTH35 | AURASTATE_FLAG_HEALTH20);
...
int32 Unit::GetAP()
{
	...
	float totalap =
		baseap * (GetFloatValue(UNIT_FIELD_ATTACK_POWER_MULTIPLIER) + 1);
	if (totalap >= 0)
...
float Unit::get_chance_to_daze(Unit * target)
{
	...
	if (target->IsPlayer())
		defense_skill =
			float (TO <
				   Player * >(target)->_GetSkillLineCurrent(SKILL_DEFENSE,
															false));
	else
	defense_skill = target->getLevel() * 5.0f;
	if (!defense_skill)
		defense_skill = 1;
	float chance_to_daze = attack_skill * 20 / defense_skill;	// if level is 
																// equal then
																// we get a
																// 20% chance
																// to daze
	chance_to_daze = chance_to_daze * std::min(target->getLevel() / 30.0f, 1.0f);	// for 
																					// targets 
																					// below 
																					// level 
																					// 30 
...
{
	if (!pHealTarget->IsPlayer() || !m_Unit->IsPlayer()
		|| pHealTarget == m_Unit)
		return;
...
void CombatStatusHandler::UpdateFlag()
{
	...
	if (n_status != m_lastStatus)
	{
		...
		else
		{
			...
			// remove any of our healers from combat too, if they are able to
			// be.
			ClearMyHealers();
...
{
	if (m_Unit->IsPlayer() && m_Unit->GetMapMgr()
		&& m_Unit->GetMapMgr()->IsCombatInProgress())
		return true;
...
void CombatStatusHandler::AddAttackTarget(const uint64 & guid)
{
	...
	// m_Unit->GetGUID());
	if (m_Unit->IsPlayer() && m_primaryAttackTarget != guid)	// players can 
																// only have
																// one attack
																// target.
	{
...
void CombatStatusHandler::ClearPrimaryAttackTarget()
{
	...
	if (m_primaryAttackTarget != 0)
	{
		...
		if (pt != NULL)
		{
			// remove from their attacker set. (if we have no longer got any
			// DoT's, etc)
			if (!IsAttacking(pt))
			{
...
bool CombatStatusHandler::IsAttacking(Unit * pTarget)
{
	...
	for (uint32 i = MAX_NEGATIVE_AURAS_EXTEDED_START;
		 i < MAX_NEGATIVE_AURAS_EXTEDED_END; ++i)
...
void CombatStatusHandler::RemoveAttackTarget(Unit * pTarget)
{
	...
	/* else printf("Cannot remove attack target "I64FMT" from "I64FMT"\n",
	   pTarget->GetGUID(), m_Unit->GetGUID()); */
}
...
void CombatStatusHandler::RemoveAttacker(Unit * pAttacker, const uint64 & guid)
{
	...
	/* else { printf("Cannot remove attacker "I64FMT" from "I64FMT"\n", guid,
	   m_Unit->GetGUID()); } */
}

void CombatStatusHandler::OnDamageDealt(Unit * pTarget)
{
	// we added an aura, or dealt some damage to a target. they need to have
	// us as an attacker, and they need to be our attack target if not.
...
{
	// If we are not in world, CombatStatusHandler::OnRemoveFromWorld() would
	// have been already called so m_attackTargets
	// and m_attackers should be empty. If it's not, something wrong happened.

	// this is a FORCED function, only use when the reference will be
	// destroyed.
...
{
	// If we are not in world, CombatStatusHandler::OnRemoveFromWorld() would
	// have been already called so m_healed should
...
bool CombatStatusHandler::IsInCombat() const const
{
	...
	case TYPEID_UNIT:
		{
			if (m_Unit->IsPet()
				&& TO < Pet * >(m_Unit)->GetPetAction() == PET_ACTION_ATTACK)
				return true;
			else if (m_Unit->IsPet())
				return m_lastStatus;
			else
				return m_Unit->GetAIInterface()->getAITargetsCount() ==
					0 ? false : true;
		}
		break;
	case TYPEID_PLAYER:
		{
			...
			for (std::list < Pet * >::iterator itr = summons.begin();
				 itr != summons.end(); ++itr)
			{
				if ((*itr)->GetPetOwner() == m_Unit
					&& (*itr)->CombatStatus.IsInCombat())
					return true;
			}
...
void Unit::CombatStatusHandler_ResetPvPTimeout()
{
	...
	sEventMgr.AddEvent(this, &Unit::CombatStatusHandler_UpdatePvPTimeout,
					   EVENT_ATTACK_TIMEOUT, 5000, 1,
					   EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	m_lock.Release();
}
...
void Unit::Heal(Unit * target, uint32 SpellId, uint32 amount)
{
	...
	if (mh != ch)
	{
		...
		Spell::SendHealSpellOnPlayer(this, target, amount, false, overheal,
									 SpellId);
...
void Unit::InheritSMMods(Unit * inherit_from)
{
	...
	if (inherit_from->SM_CriticalChance)
	{
		...
		memcpy(SM_CriticalChance, inherit_from->SM_CriticalChance,
			   sizeof(int) * SPELL_GROUPS);
	}
	...
	if (inherit_from->SM_PRadius)
	{
		...
		memcpy(SM_PRadius, inherit_from->SM_PRadius,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FRadius)
	{
		...
		memcpy(SM_FRadius, inherit_from->SM_FRadius,
			   sizeof(int) * SPELL_GROUPS);
	}
	...
	if (inherit_from->SM_PCastTime)
	{
		...
		memcpy(SM_PCastTime, inherit_from->SM_PCastTime,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FCastTime)
	{
		...
		memcpy(SM_FCastTime, inherit_from->SM_FCastTime,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PCriticalDamage)
	{
		...
		memcpy(SM_PCriticalDamage, inherit_from->SM_PCriticalDamage,
			   sizeof(int) * SPELL_GROUPS);
	}
	...
	if (inherit_from->SM_FEffect1_Bonus)
	{
		...
		memcpy(SM_FEffect1_Bonus, inherit_from->SM_FEffect1_Bonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PEffect1_Bonus)
	{
		...
		memcpy(SM_PEffect1_Bonus, inherit_from->SM_PEffect1_Bonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FEffect2_Bonus)
	{
		...
		memcpy(SM_FEffect2_Bonus, inherit_from->SM_FEffect2_Bonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PEffect2_Bonus)
	{
		...
		memcpy(SM_PEffect2_Bonus, inherit_from->SM_PEffect2_Bonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FEffect3_Bonus)
	{
		...
		memcpy(SM_FEffect3_Bonus, inherit_from->SM_FEffect3_Bonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PEffect3_Bonus)
	{
		...
		memcpy(SM_PEffect3_Bonus, inherit_from->SM_PEffect3_Bonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FEffectBonus)
	{
		...
		memcpy(SM_FEffectBonus, inherit_from->SM_FEffectBonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PEffectBonus)
	{
		...
		memcpy(SM_PEffectBonus, inherit_from->SM_PEffectBonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FDamageBonus)
	{
		...
		memcpy(SM_FDamageBonus, inherit_from->SM_FDamageBonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PDamageBonus)
	{
		...
		memcpy(SM_PDamageBonus, inherit_from->SM_PDamageBonus,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PMiscEffect)
	{
		...
		memcpy(SM_PMiscEffect, inherit_from->SM_PMiscEffect,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FMiscEffect)
	{
		...
		memcpy(SM_FMiscEffect, inherit_from->SM_FMiscEffect,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FHitchance)
	{
		...
		memcpy(SM_FHitchance, inherit_from->SM_FHitchance,
			   sizeof(int) * SPELL_GROUPS);
	}
	...
	if (inherit_from->SM_FAdditionalTargets)
	{
		...
		memcpy(SM_FAdditionalTargets, inherit_from->SM_FAdditionalTargets,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PJumpReduce)
	{
		...
		memcpy(SM_PJumpReduce, inherit_from->SM_PJumpReduce,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FGlobalCooldown)
	{
		...
		memcpy(SM_FGlobalCooldown, inherit_from->SM_FGlobalCooldown,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PGlobalCooldown)
	{
		...
		memcpy(SM_PGlobalCooldown, inherit_from->SM_PGlobalCooldown,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PNonInterrupt)
	{
		...
		memcpy(SM_PNonInterrupt, inherit_from->SM_PNonInterrupt,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FPenalty)
	{
		...
		memcpy(SM_FPenalty, inherit_from->SM_FPenalty,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PPenalty)
	{
		...
		memcpy(SM_PPenalty, inherit_from->SM_PPenalty,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FCooldownTime)
	{
		...
		memcpy(SM_FCooldownTime, inherit_from->SM_FCooldownTime,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PCooldownTime)
	{
		...
		memcpy(SM_PCooldownTime, inherit_from->SM_PCooldownTime,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FChanceOfSuccess)
	{
		...
		memcpy(SM_FChanceOfSuccess, inherit_from->SM_FChanceOfSuccess,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FAmptitude)
	{
		...
		memcpy(SM_FAmptitude, inherit_from->SM_FAmptitude,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PAmptitude)
	{
		...
		memcpy(SM_PAmptitude, inherit_from->SM_PAmptitude,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FRezist_dispell)
	{
		...
		memcpy(SM_FRezist_dispell, inherit_from->SM_FRezist_dispell,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PRezist_dispell)
	{
		...
		memcpy(SM_PRezist_dispell, inherit_from->SM_PRezist_dispell,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FCharges)
	{
		...
		memcpy(SM_FCharges, inherit_from->SM_FCharges,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PCharges)
	{
		...
		memcpy(SM_PCharges, inherit_from->SM_PCharges,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_FThreat)
	{
		...
		memcpy(SM_FThreat, inherit_from->SM_FThreat,
			   sizeof(int) * SPELL_GROUPS);
	}
	if (inherit_from->SM_PThreat)
	{
		...
		memcpy(SM_PThreat, inherit_from->SM_PThreat,
			   sizeof(int) * SPELL_GROUPS);
	}
}
...
void CombatStatusHandler::TryToClearAttackTargets()
{
	...
	if (m_Unit->IsPlayer())
		TO < Player * >(m_Unit)->RemoveFlag(PLAYER_FLAGS,
											PLAYER_FLAG_UNKNOWN2);
...
{
	/* 
	   if(ptr) ptr->cancel(); else */
	if (m_currentSpell)
	{
		// this logically might seem a little bit twisted
		// crash situation : an already deleted spell will be called to get
		// canceled by eventmanager
		// solution : We should not delay spell canceling more then second
		// spell canceling.
		// problem : might remove spells that should not be removed. Not sure
		// about it :(
...
void Unit::DispelAll(bool positive)
{
	...
	{
		if (m_auras[i] != NULL)
			if ((m_auras[i]->IsPositive() && positive)
				|| !m_auras[i]->IsPositive())
				m_auras[i]->Remove();
	}
}

/* bool Unit::RemoveAllAurasByMechanic (renamed from
   MechanicImmunityMassDispel) - Removes all auras on this unit that are of a
   specific mechanic. - Useful for things like.. Apply Aura: Immune Mechanic,
   where existing (de)buffs are *always supposed* to be removed. - I'm not
   sure if this goes here under unit. * Arguments: - uint32 MechanicType *

   * Returns; - False if no buffs were dispelled, true if more than 0 were
   dispelled. */
bool Unit::RemoveAllAurasByMechanic(uint32 MechanicType, uint32 MaxDispel =
									-1, bool HostileOnly = true)
{
	// sLog.outString( "Unit::MechanicImmunityMassDispel called, mechanic: %u" 
	// , MechanicType );
	uint32 DispelCount = 0;
	for (uint32 x = (HostileOnly ? MAX_NEGATIVE_AURAS_EXTEDED_START : MAX_POSITIVE_AURAS_EXTEDED_START); x < MAX_REMOVABLE_AURAS_END; x++)	// If 
																																			// HostileOnly 
																																			// = 
																																			// 1, 
																																			// then 
																																			...
																																			// (all)
	{
		...
		if (m_auras[x])
		{
			if (m_auras[x]->GetSpellProto()->MechanicsType == MechanicType)	// Remove 
																			// all 
																			// mechanics 
																			// of 
																			// type 
																			...
																			// boom)
			{
				// sLog.outString( "Removed aura. [AuraSlot %u, SpellId %u]" , 
				// x , m_auras[x]->GetSpellId() );
				// TODO: Stop moving if fear was removed.
				m_auras[x]->Remove();	// EZ-Remove
				DispelCount++;
			}
			else if (MechanicType == MECHANIC_ENSNARED)	// if got immunity for 
														// slow, remove some
														// that are not in the 
														// mechanics
			{
				...
				{
					// SNARE + ROOT
					if (m_auras[x]->GetSpellProto()->EffectApplyAuraName[i] ==
						SPELL_AURA_MOD_DECREASE_SPEED
						|| m_auras[x]->GetSpellProto()->
						EffectApplyAuraName[i] == SPELL_AURA_MOD_ROOT)
					{
						m_auras[x]->Remove();
						break;
					}
		 ...
		 x < MAX_REMOVABLE_AURAS_END; x++)
	{
		if (m_auras[x] != NULL)
		{
			if (m_auras[x]->GetSpellProto()->MechanicsType == MECHANIC_ROOTED
				|| m_auras[x]->GetSpellProto()->MechanicsType ==
				MECHANIC_ENSNARED
				|| m_auras[x]->GetSpellProto()->MechanicsType ==
				MECHANIC_DAZED)
...
void Unit::setAttackTimer(int32 time, bool offhand)
{
	if (!time)
		time =
			offhand ? m_uint32Values[UNIT_FIELD_BASEATTACKTIME +
									 1] :
			m_uint32Values[UNIT_FIELD_BASEATTACKTIME];
...
{
	delete m_aiInterface;		// be careful when you do this. Might screw
								// unit states !
...
{
	DisplayBounding *entry =
		DisplayBoundingStorage.
		LookupEntry(GetUInt32Value(UNIT_FIELD_DISPLAYID));

	// TODO: if has mount, grab mount model and add the z value of attachment
	// 0
	if (entry)
...
void Unit::AggroPvPGuards()
{
		 ...
		 i != GetInRangeSetEnd(); ++i)
	{
		...
		{
			tmpUnit = TO < Unit * >(*i);
			if (tmpUnit->GetAIInterface()
				&& tmpUnit->GetAIInterface()->m_isNeutralGuard
				&& CalcDistance(tmpUnit) <= (50.0f * 50.0f))
			{
...
void Unit::EventChill(Unit * proc_target, bool is_victim)
{
	...
	{
		if (t_trigger_on_chill_chance < 100
			&& !Rand(t_trigger_on_chill_chance))
			return;
...
{
	ExtraStrike *es;
	for (std::list < ExtraStrike * >::iterator i =
		 m_extraStrikeTargets.begin(); i != m_extraStrikeTargets.end(); i++)
	{
		es = *i;
...
uint32 Unit::DoDamageSplitTarget(uint32 res, uint32 school_type,
								 bool melee_dmg)
{
	Unit *splittarget;
	...
	splittarget =
		(GetMapMgr() != NULL) ? GetMapMgr()->GetUnit(ds->m_target) : NULL;
	if (splittarget != NULL && res > 0)
	{
		...
		if (splitdamage)
		{
			...
			if (melee_dmg)
			{
				...
				SendAttackerStateUpdate(this, splittarget, &sdmg, splitdamage,
										0, 0, 0, ATTACK);
			}
			else
			{
				SendSpellNonMeleeDamageLog(this, splittarget, ds->m_spellId,
										   splitdamage,
										   static_cast < uint8 > (school_type),
										   0, 0, true, 0, 0, true);
			}
		}
	}
...
void Unit::RemoveReflect(uint32 spellid, bool apply)
{
	...
	*/
	for (std::list < struct ReflectSpellSchool * >::iterator i =
		 m_reflectSpellSchool.begin(); i != m_reflectSpellSchool.end();)
	...
	if (apply && spellid == 23920 && IsPlayer()
		&& HasAurasWithNameHash(SPELL_HASH_IMPROVED_SPELL_REFLECTION))
	{
		...
		if (pGroup != NULL)
		{
			...
			for (uint32 i = 0; i < pGroup->GetSubGroupCount(); ++i)
			{
				...
				for (GroupMembersSet::iterator itr =
					 subGroup->GetGroupMembersBegin();
					 itr != subGroup->GetGroupMembersEnd() && targets > 0;
					 ++itr)
				{
					Player *member = (*itr)->m_loggedInPlayer;
					if (member == NULL || member == pPlayer
						|| !member->IsInWorld() || !member->isAlive()
						|| member->HasAura(59725))
						continue;
	...
	if (!apply && spellid == 59725 && IsPlayer())
	{
		...
		if (pGroup != NULL)
		{
			...
			{
				for (GroupMembersSet::iterator itr =
					 pGroup->GetSubGroup(i)->GetGroupMembersBegin();
					 itr != pGroup->GetSubGroup(i)->GetGroupMembersEnd();
					 ++itr)
				{
					Player *member = (*itr)->m_loggedInPlayer;
					if (member == NULL)
...
{
	uint32 amount = GetUInt32Value(UNIT_FIELD_POWER1 + GetPowerType());	// save 
																		// the 
																		// amount, 
																		// so
																		// we
	...
	data << amount;
	// This was added in revision 1726.  Is it necessary? To me, it seems to
	// just be sending the packet twice.
	// If it is needed for something, put it back in I guess.
	// CopyAndSendDelayedPacket(&data);
	SendMessageToSet(&data, self);

	// VLack: On 3.1.3, create and send a field update packet to everyone
	// else, as this is the only way to update their GUI with the power
	// values.
	WorldPacket *pkt =
		BuildFieldUpdatePacket(UNIT_FIELD_POWER1 + GetPowerType(), amount);
...
{
	Arcemu::Util::ARCEMU_ASSERT(pet->GetPetOwner()->GetGUID() == GetGUID()
								&& !pet->IsInWorld());
...
{
	if (IsTagged() && !IsPet() && !isCritter() && !(IsPlayer() && !IsInBg())
		&& GetCreatedByGUID() == 0)
		return true;
	else
		return false;
...
SpellProc *Unit::AddProcTriggerSpell(SpellEntry * spell,
									 SpellEntry * orig_spell, uint64 caster,
									 uint32 procChance, uint32 procFlags,
									 uint32 procCharges,
									 uint32 * groupRelation,
									 uint32 * procClassMask, Object * obj)
{
	SpellProc *sp = NULL;
	if (spell != NULL)
	...
	sp = sSpellProcMgr.NewSpellProc(this, spell, orig_spell, caster,
									procChance, procFlags, procCharges,
									groupRelation, procClassMask, obj);
	if (sp == NULL)
	{
		if (orig_spell != NULL)
			LOG_ERROR
				("Spell id %u tried to add a non-existent spell to Unit %p as SpellProc",
				 orig_spell->Id, this);
		else
			LOG_ERROR
				("Something tried to add a non-existent spell to Unit %p as SpellProc",
				 this);
		return NULL;
	}
...
SpellProc *Unit::AddProcTriggerSpell(uint32 spell_id, uint32 orig_spell_id,
									 uint64 caster, uint32 procChance,
									 uint32 procFlags, uint32 procCharges,
									 uint32 * groupRelation,
									 uint32 * procClassMask, Object * obj)
{
	return AddProcTriggerSpell(dbcSpell.LookupEntryForced(spell_id),
							   dbcSpell.LookupEntryForced(orig_spell_id),
							   caster, procChance, procFlags, procCharges,
							   groupRelation, procClassMask, obj);
}

SpellProc *Unit::AddProcTriggerSpell(SpellEntry * sp, uint64 caster,
									 uint32 * groupRelation,
									 uint32 * procClassMask, Object * obj)
{
	return AddProcTriggerSpell(sp, sp, caster, sp->procChance, sp->procFlags,
							   sp->procCharges, groupRelation, procClassMask,
							   obj);
}
...
SpellProc *Unit::GetProcTriggerSpell(uint32 spellId, uint64 casterGuid)
{
	...
	{
		SpellProc *sp = *itr;
		if (sp->mSpell->Id == spellId
			&& (casterGuid == 0 || sp->mCaster == casterGuid))
			return sp;
	}
...
void Unit::RemoveProcTriggerSpell(uint32 spellId, uint64 casterGuid,
								  uint64 misc)
{
	for (std::list < SpellProc * >::iterator itr = m_procSpells.begin();
		 itr != m_procSpells.end(); ++itr)
...
void Unit::TakeDamage(Unit * pAttacker, uint32 damage, uint32 spellid,
					  bool no_remove_auras)
{
}
...
void Unit::SendPeriodicAuraLog(const WoWGuid & CasterGUID,
							   const WoWGuid & TargetGUID, uint32 SpellID,
							   uint32 School, uint32 Amount, uint32 abs_dmg,
							   uint32 resisted_damage, uint32 Flags,
							   bool is_critical)
{
	...
	data << uint32(0);			// cebernic: unknown?? needs more research,
								// but it should fix unknown damage type with
								// suffered.
...
void Unit::SendPeriodicHealAuraLog(const WoWGuid & CasterGUID,
								   const WoWGuid & TargetGUID, uint32 SpellID,
								   uint32 healed, uint32 over_healed,
								   bool is_critical)
{
	...
	data << uint32(0);			// I don't know what it is. maybe something
								// related to absorbed heal?
	data << uint8(is_critical);
...
void Unit::Phase(uint8 command, uint32 newphase)
{
	...
	for (std::set < Object * >::iterator itr = m_objectsInRange.begin();
		 itr != m_objectsInRange.end(); ++itr)
	{
		if ((*itr)->IsUnit())
...
uint64 Unit::GetCurrentUnitForSingleTargetAura(uint32 * name_hashes,
											   uint32 * index)
{
	UniqueAuraTargetMap::iterator itr;
...
bool Unit::InParty(Unit * u)
{
	...
	if (p->GetGroup() != NULL && uFrom->GetGroup() != NULL
		&& p->GetGroup() == uFrom->GetGroup()
		&& p->GetSubGroup() == uFrom->GetSubGroup())
		return true;
...
bool Unit::IsCriticalDamageForSpell(Object * victim, SpellEntry * spell)
{
	...
	if (spell->is_ranged_spell)
	{
		if (IsPlayer())
		{
			...
			if (victim->IsUnit())
				CritChance +=
					(float)(TO_UNIT(victim)->
							AttackerCritChanceMod[spell->School]);
		}
		else
			CritChance = 5.0f;	// static value for mobs.. not blizzlike, but
								// an unfinished formula is not fatal :)
	...
	else if (spell->is_melee_spell)
	{
		...
		if (victim->IsPlayer())
		{
			CritChance += TO_PLAYER(victim)->res_R_crit_get();	// this could
																// be ability
																// but in that 
																// case we
																// overwrite
	...
	else
	{
		...
		{
			CritChance +=
				TO_UNIT(victim)->AttackerCritChanceMod[spell->School];

			if (IsPlayer()
				&& (TO_UNIT(victim)->m_rooted - TO_UNIT(victim)->m_stunned))
	...
	// HACK!!!
	Aura *fs = NULL;
	if (victim->IsUnit() && spell->NameHash == SPELL_HASH_LAVA_BURST
		&& (fs =
			TO_UNIT(victim)->FindAuraByNameHash(SPELL_HASH_FLAME_SHOCK)) !=
		NULL)
	{
		result = true;
...
float Unit::GetCriticalDamageBonusForSpell(Object * victim, SpellEntry * spell,
										   float amount)
{
	...
	if (critical_bonus > 0)
	{
		...
		float b;
		if (spell->School == 0 || spell->is_melee_spell || spell->is_ranged_spell)	// physical 
																					// || 
																					// hackfix 
																					// SoCommand/JoCommand
	...
	if (victim->IsPlayer())
	{
		// res = res*(1.0f-2.0f*TO< Player*
		// >(pVictim)->CalcRating(PLAYER_RATING_MODIFIER_MELEE_CRIT_RESISTANCE));
		// Resilience is a special new rating which was created to reduce the
		// effects of critical hits against your character.
		// It has two components; it reduces the chance you will be critically 
		// hit by x%,
		// and it reduces the damage dealt to you by critical hits by 2x%. x
		// is the percentage resilience granted by a given resilience rating.
		// It is believed that resilience also functions against spell crits,
		// though it's worth noting that NPC mobs cannot get critical hits
		// with spells.

		float dmg_reduction_pct =
			2 *
			TO_PLAYER(victim)->
			CalcRating(PLAYER_RATING_MODIFIER_MELEE_CRIT_RESILIENCE) / 100.0f;

		if (dmg_reduction_pct > 1.0f)
			dmg_reduction_pct = 1.0f;	// we cannot resist more then he is
										// criticalling us, there is no point
										// of the critical then :P

		amount -= amount * dmg_reduction_pct;
	}

	if (victim->IsCreature()
		&& TO_CREATURE(victim)->GetCreatureInfo()->Rank != ELITE_WORLDBOSS)
...
bool Unit::IsCriticalHealForSpell(Object * victim, SpellEntry * spell)
{
	...
	crit_chance =
		float2int32(this->spellcritperc +
					this->SpellCritChanceSchool[spell->School]);

	// Sacred Shield
	if (victim->IsUnit()
		&& TO_UNIT(victim)->HasAurasWithNameHash(SPELL_HASH_SACRED_SHIELD)
		&& spell->NameHash == SPELL_HASH_FLASH_OF_LIGHT)
		crit_chance += 50;

	if (spell->SpellGroupType)
		SM_FIValue(this->SM_CriticalChance, &crit_chance,
				   spell->SpellGroupType);

	return Rand(crit_chance);
}

float Unit::GetCriticalHealBonusForSpell(Object * victim, SpellEntry * spell,
										 float amount)
{
	int32 critical_bonus = 100;
	if (spell->SpellGroupType)
		SM_FIValue(this->SM_PCriticalDamage, &critical_bonus,
				   spell->SpellGroupType);
...
uint32 Unit::GetAuraCountWithDispelType(uint32 dispel_type, uint64 guid)
{
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; ++x)
	{
		...
		if (m_auras[x]->GetSpellProto()->DispelType == dispel_type
			&& (guid == 0 || m_auras[x]->GetCasterGUID() == guid))
			result++;
	}
...
{
	// This is in unit and not creature because players who are mind
	// controlled must use this.
	if (caster == NULL)
		caster = this;
	float angle =
		calcRadAngle(caster->GetPositionX(), caster->GetPositionY(),
	...
	float waterz;
	uint32 watertype;
	GetMapMgr()->GetLiquidInfo(destx, desty, GetPositionZ() + 2, waterz,
							   watertype);
	...
	if (nav != NULL)
	{
		...
		if (!GetAIInterface()->CanCreatePath(destx, desty, destz))
		{
			...
			float start[3] =
				{ GetPositionY(), GetPositionZ() + 0.5f, GetPositionX() };
			float end[3] = { desty, destz + 0.5f, destx };
			float extents[3] = { 3, 5, 3 };
			dtQueryFilter filter;
			filter.
				setIncludeFlags(NAV_GROUND | NAV_WATER | NAV_SLIME |
								NAV_MAGMA);

			dtPolyRef startref;
			nav->query->findNearestPoly(start, extents, &filter, &startref,
										NULL);
			...
			nav->query->moveAlongSurface(startref, start, end, &filter, result,
										 visited, &numvisited,
										 MAX_PATH_LENGTH);
			nav->query->getPolyHeight(visited[numvisited - 1], result,
									  &result[1]);
	...
	else						// test against vmap if mmap isn't available
	{
		...
		if (CollideInterface.
			GetFirstPoint(GetMapId(), GetPositionX(), GetPositionY(),
						  GetPositionZ() + 2, destx, desty, destz + 2, testx,
						  testy, testz, -0.5f))
		{
			// hit something
			destx = testx;
