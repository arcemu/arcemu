...
void Unit::Update(uint32 p_time)
{
	...
	if (!IsDead())
	{
		...
		/* //if health changed since last time. Would be perfect if it would work for creatures too :) if(m_updateMask.GetBit(UNIT_FIELD_HEALTH)) EventHealthChangeSinceLastUpdate(); */
	}
}
...
uint32 Unit::HandleProc(uint32 flag, Unit * victim, SpellEntry * CastingSpell, bool is_triggered, uint32 dmg, uint32 abs, uint32 weapon_damage_type)
{
	...
	for (itr = m_procSpells.begin(); itr != m_procSpells.end();)	// Proc Trigger Spells for Victim
	{
		...
		if (this->IsPlayer() && (spe->NameHash == SPELL_HASH_MAGTHERIDON_MELEE_TRINKET || spe->NameHash == SPELL_HASH_ROMULO_S_POISON || spe->NameHash == SPELL_HASH_BLACK_TEMPLE_MELEE_TRINKET || spe->NameHash == SPELL_HASH_FROSTBRAND_ATTACK || spellId == 16870))
		{
			float ppm = 1.0f;
			switch (spe->NameHash)
			{
			...
			case 16953:		// Druid - Blood Frenzy Proc
				{
					if (!IsPlayer() || !CastingSpell)
						continue;
					Player *p = TO < Player * >(this);
					if (p->GetShapeShift() != FORM_CAT || (CastingSpell->NameHash != SPELL_HASH_CLAW && CastingSpell->NameHash != SPELL_HASH_RAKE && CastingSpell->NameHash != SPELL_HASH_RAVAGE && CastingSpell->NameHash != SPELL_HASH_SHRED))
						continue;
				}
				break;
			case 14189:		// Seal Fate
				{
					if (!this->IsPlayer() || !CastingSpell || CastingSpell->Id == 14189 || CastingSpell->Id == 16953 || CastingSpell->Id == 16959)
						continue;
					if (CastingSpell->Effect[0] != SPELL_EFFECT_ADD_COMBO_POINTS && CastingSpell->Effect[1] != SPELL_EFFECT_ADD_COMBO_POINTS && CastingSpell->Effect[2] != SPELL_EFFECT_ADD_COMBO_POINTS && CastingSpell->NameHash != SPELL_HASH_MANGLE__CAT_)
						continue;
				}
				break;
			...
			case 37309:		// Bloodlust
				{
					if (!this->IsPlayer())
						continue;
					if (TO < Player * >(this)->GetShapeShift() != FORM_BEAR || TO < Player * >(this)->GetShapeShift() != FORM_DIREBEAR)
						continue;
				}
				break;
			...
			case 37117:
				{
					...
					if (!mPlayer->IsInFeralForm() || (mPlayer->GetShapeShift() != FORM_CAT && mPlayer->GetShapeShift() != FORM_BEAR && mPlayer->GetShapeShift() != FORM_DIREBEAR))
						continue;
				}
				break;
			...
			case 47930:
				{
					if (CastingSpell == NULL)
						continue;
					if (CastingSpell->NameHash != SPELL_HASH_PENANCE && CastingSpell->NameHash != SPELL_HASH_FLASH_HEAL && CastingSpell->NameHash != SPELL_HASH_GREATER_HEAL)
						continue;
				}
				break;
			...
			case 44401:
				{
					...
					if (CastingSpell->NameHash != SPELL_HASH_ARCANE_BLAST && CastingSpell->NameHash != SPELL_HASH_ARCANE_BARRAGE && CastingSpell->NameHash != SPELL_HASH_FIREBALL && CastingSpell->NameHash != SPELL_HASH_FROSTBOLT && CastingSpell->NameHash != SPELL_HASH_FROSTFIRE_BOLT)
						continue;
				}
				break;
			...
			case 53489:
				{
					if (CastingSpell == NULL)
						continue;
					if (CastingSpell->Id != 53408 && CastingSpell->Id != 53407 && CastingSpell->Id != 20271 && CastingSpell->NameHash != SPELL_HASH_DIVINE_STORM && CastingSpell->NameHash != SPELL_HASH_CRUSADER_STRIKE)
						continue;
				}
				break;
			...
			case 43838:
				{
					...
					if (CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_JUSTICE && CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_LIGHT && CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_WISDOM && CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_RIGHTEOUSNESS && CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_BLOOD && CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_VENGEANCE && CastingSpell->NameHash != SPELL_HASH_JUDGEMENT_OF_COMMAND)
						continue;
				}
				break;
			case 43837:
			...
			if ((iter2->second.ProcFlag & flag))
			{
				...
				if (CastingSpell)
				{
					...
					case 43339:	// Shaman - Shamanist Focus
						{
							if (CastingSpell->NameHash != SPELL_HASH_EARTH_SHOCK && CastingSpell->NameHash != SPELL_HASH_FLAME_SHOCK && CastingSpell->NameHash != SPELL_HASH_FROST_SHOCK)
								continue;
						}
						break;
...
void Unit::RegeneratePower(bool isinterrupted)
{
	...
	if (this->IsPlayer())
	{
		...
		/* 
		   ...
		   There is a problem here for druids. Druids when shapeshifted into bear have 2 power with different regen timers a) Mana (which regenerates while shapeshifted b) Rage

		   Mana has a regen timer of 2 seconds Rage has a regen timer of 3 seconds

		   I think the only viable way of fixing this is to have 2 different timers to check each individual power.
...
void Unit::Strike(Unit * pVictim, uint32 weapon_damage_type, SpellEntry * ability, int32 add_damage, int32 pct_dmg_mod, uint32 exclusive_damage, bool disable_proc, bool skip_hit_check, bool force_crit)
{
	...
	if (pVictim->IsCreature())
	{
		if (pVictim->GetAIInterface() && (pVictim->GetAIInterface()->getAIState() == STATE_EVADE || (pVictim->GetAIInterface()->GetIsSoulLinked() && pVictim->GetAIInterface()->getSoullinkedWith() != this)))
		{
			vstate = EVADE;
			realdamage = 0;
			dmg.full_damage = 0;
	...
	if (realdamage > 0 && ability == 0)
	{
		if (IsPlayer() && TO < Player * >(this)->m_onStrikeSpells.size())
		{
			...
			for (map < SpellEntry *, pair < uint32, uint32 > >::iterator itr = TO < Player * >(this)->m_onStrikeSpells.begin(); itr != TO < Player * >(this)->m_onStrikeSpells.end(); ++itr)
			{
				if (itr->second.first)
				{
...
void Unit::AddAura(Aura * aur)
{
		...
		if (aur->GetSpellProto()->always_apply == false)
		{
			...
			for (uint32 x = StartCheck; x < CheckLimit; x++)
			{
				if (m_auras[x])
				{
					...
					{
						if (!aur->IsPositive() && m_auras[x]->m_casterGuid != aur->m_casterGuid && (m_auras[x]->GetSpellProto()->c_is_flags & SPELL_FLAG_IS_MAXSTACK_FOR_DEBUFF) == 0)
						{
							continue;
						}
						AlreadyApplied++;
	...
	{
		sEventMgr.AddEvent(aur, &Aura::Remove, EVENT_AURA_REMOVE, aur->GetDuration() + 500, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT | EVENT_FLAG_DELETES_OBJECT);
	}
	...
	if (!aur->IsPositive() && aur->IsCombatStateAffecting())	// Creature
	{
		...
		/* if(isAlive() && CanAgroHash(aur->m_spellProto->NameHash)) //no threat for hunter's mark { Unit * pCaster = aur->GetUnitCaster(); if(!pCaster) return;
...
Aura *Unit::FindAuraWithAuraEffect(uint32 effect, uint32 * x)
{
	...
	{
		aura = m_auras[*x];
		if (aura != NULL && ((aura->GetSpellProto()->Effect[0] == SPELL_EFFECT_APPLY_AURA && aura->GetSpellProto()->EffectApplyAuraName[0] == effect) || (aura->GetSpellProto()->Effect[1] == SPELL_EFFECT_APPLY_AURA && aura->GetSpellProto()->EffectApplyAuraName[1] == effect) || (aura->GetSpellProto()->Effect[2] == SPELL_EFFECT_APPLY_AURA && aura->GetSpellProto()->EffectApplyAuraName[2] == effect)))
			return aura;
	}
	...
	for (uint32 x = MAX_TOTAL_AURAS_START; x < MAX_TOTAL_AURAS_END; x++)
	{
		...
		if (aura != NULL && aura->GetSpellProto()->NameHash == name_hash)
		{
			...
			if ((aura_sp->Effect[0] == proto->Effect[0] && (aura_sp->Effect[0] != SPELL_EFFECT_APPLY_AURA || aura_sp->EffectApplyAuraName[0] == proto->EffectApplyAuraName[0])) &&
				(aura_sp->Effect[1] == proto->Effect[1] && (aura_sp->Effect[1] != SPELL_EFFECT_APPLY_AURA || aura_sp->EffectApplyAuraName[1] == proto->EffectApplyAuraName[1])) && (aura_sp->Effect[2] == proto->Effect[2] && (aura_sp->Effect[2] != SPELL_EFFECT_APPLY_AURA || aura_sp->EffectApplyAuraName[2] == proto->EffectApplyAuraName[2])))
			{
...
AuraCheckResponse Unit::AuraCheck(SpellEntry * proto, Aura * aur, Object * caster)
{
	...
	if (aur->GetSpellProto()->NameHash == proto->NameHash)
	{
		// we've got an aura with the same name as the one we're trying to apply
		// but first we check if it has the same effects
		if ((aura_sp->Effect[0] == proto->Effect[0] && (aura_sp->Effect[0] != SPELL_EFFECT_APPLY_AURA || aura_sp->EffectApplyAuraName[0] == proto->EffectApplyAuraName[0])) && (aura_sp->Effect[1] == proto->Effect[1] && (aura_sp->Effect[1] != SPELL_EFFECT_APPLY_AURA || aura_sp->EffectApplyAuraName[1] == proto->EffectApplyAuraName[1])) && (aura_sp->Effect[2] == proto->Effect[2] && (aura_sp->Effect[2] != SPELL_EFFECT_APPLY_AURA || aura_sp->EffectApplyAuraName[2] == proto->EffectApplyAuraName[2])))
		{
...
/* bool Unit::RemoveAllAurasByMechanic (renamed from MechanicImmunityMassDispel) - Removes all auras on this unit that are of a specific mechanic. - Useful for things like.. Apply Aura: Immune Mechanic, where existing (de)buffs are *always supposed* to be removed. - I'm not sure if this goes here under unit. * Arguments: - uint32 MechanicType *
...
void Unit::RemoveAllMovementImpairing()
{
	for (uint32 x = MAX_NEGATIVE_AURAS_EXTEDED_START; x < MAX_REMOVABLE_AURAS_END; x++)
	{
		if (m_auras[x] != NULL)
		{
			if (m_auras[x]->GetSpellProto()->MechanicsType == MECHANIC_ROOTED || m_auras[x]->GetSpellProto()->MechanicsType == MECHANIC_ENSNARED || m_auras[x]->GetSpellProto()->MechanicsType == MECHANIC_DAZED)
