...
void TotemAI::HandleEvent( uint32 event, Unit* pUnit, uint32 misc1 )
{
	//load totem spells from proto when we get in world.
	if(event == EVENT_ENTERWORLD)
	{
		CreatureProto * proto = TO_CREATURE(m_Self)->GetProto();
		Spell_reserveStorage(proto->spells.size() );
