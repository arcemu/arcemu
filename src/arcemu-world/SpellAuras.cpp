...
void Aura::EventPeriodicDamage(uint32 amount)
{
	...
	if(m_target->GetGUID() != m_casterGuid)	//don't use resist when cast on self-- this is some internal stuff
	{
		if(c != NULL)
		{
			...
			LOG_BASIC("res BEFORE assigning the SpellDamageBonus = %f", res);
			res += bonus;
			...
			res += reduce_damage;
			LOG_BASIC("res AFTER assigning the DamageTakenPctMod = %f", res);
			}
			...
			else
			{
				...
				res *= summaryPCTmod;
				LOG_BASIC("res AFTER assigning the Resilience reduction = %f", res);
				if(res < 0.0f)
					res = 0.0f;
			}
