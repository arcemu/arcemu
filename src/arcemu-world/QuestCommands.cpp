...
bool ChatHandler::HandleQuestShowLogCommand(const char * args, WorldSession * m_session)
{
	uint32 i;
	QuestLogEntry *qle;
	...
	if(chr == NULL)
		{
         RedSystemMessage(m_session, "Select a character.");
         return false;
		}
	else
		{
         for( i = 0; i < 25; ++i )
			{
				if((qle = chr->GetQuestLogInSlot(i)) != 0)
				{
					uint32 questId = qle->GetQuest()->id;
					uint32 questLevel = qle->GetQuest()->questlevel;
					const char * questTitle = qle->GetQuest()->title;
					SystemMessage(m_session, "|cff808080|Hquest:%u:%u|h[%s]|h|r Quest slot %u", questId, questLevel, questTitle, i);
				}
			}
		}	
	return true;
...
bool ChatHandler::HandleQuestRequirementCommand(const char * args, WorldSession * m_session)
{
   uint32 i, j;
	uint32 questId = 0;
	uint32 type = 0;
   uint32 questslot = 0;
	sscanf(args, "%u %u", uint32 (&questId), uint32 (&type));
	Player * chr = getSelectedChar(m_session, true);

   if(chr == NULL)
   {
		RedSystemMessage(m_session, "Select a player");
		return false;
   }

   Quest * qst = QuestStorage.LookupEntry(questId);
	QuestLogEntry * qle = chr->GetQuestLogForEntry(questId);
	QuestLogEntry * qle2 = new QuestLogEntry();

   if(qle == NULL)
   {
      RedSystemMessage(m_session, "Quest not found in player's quest log.");
      return false;
   }
   else
   {
      questslot = qle->GetSlot();
   }

   if(type == NULL)
   {
		RedSystemMessage(m_session, "Enter assistance type.");
		return false;
   }
   
	switch(type)
   {
      case 1://restarts quest
      {
         for(i = 0; i < 6; ++i)
         {
            if(qst->required_item[i] != qst->srcitem)
            {
               if(chr->GetItemInterface()->GetItemCount(qst->required_item[i]) > qst->required_itemcount[i])
               {
                  chr->GetItemInterface()->RemoveItemAmt(qst->required_item[i],qst->required_itemcount[i]);
               }
               else
               {
                  chr->GetItemInterface()->RemoveItemAmt(qst->required_item[i],chr->GetItemInterface()->GetItemCount(qst->required_item[i]));
               }
            }
         }
         RemoveQuestFromPlayer(chr, qst);
         qle2->Init(qst, chr, questslot);
         qle2->UpdatePlayerFields();
         chr->SaveToDB(false);
      }break;
      case 2://items required for quest completion
      {
         for(i = 0; i < 6; ++i)
         {
            if(qst->required_item[i] != 0)
            {
               if(chr->GetItemInterface()->GetItemCount(qst->required_item[i]) < qst->required_itemcount[i])
               {
                  chr->GetItemInterface()->AddItemById( qst->required_item[i],qst->required_itemcount[i] - chr->GetItemInterface()->GetItemCount(qst->required_item[i]), 0 );
      ...
      case 3://sets GOs as activated for quest completion
      {
         for(i = 0; i < 4; ++i)
         {
            if(qst->required_mob[i] != 0 && qst->required_mob[i] < 0)
            {
               for(j = 0; qle->GetMobCount(i) < qst->required_mobcount[i]; ++j)
               {
                  qle->IncrementMobCount( i );
                  CALL_QUESTSCRIPT_EVENT( qle, OnGameObjectActivate )( qst->required_mob[i], chr, qle );
                  if( qle->CanBeFinished() )
                  {
                     qle->SendQuestComplete();
                  }
      ...
      case 4://sets mobs as killed for quest completion
      {
         for(i = 0; i < 4; ++i)
         {
            if(qst->required_mob[i] != 0 && qst->required_mob[i] > 0)
      ...
      case 5://sets spells as cast for quest completion
      {
         for(i = 0; i < 4; ++i)
         {
            if(qst->required_spell[i] != 0)
            {
               for(j = 0; qle->GetMobCount(i) < qst->required_mobcount[i]; ++j)
               {
                  qle->IncrementMobCount( i );
      ...
      case 6://sets emote as complete for quest completion
      {
         for(i = 0; i < 4; ++i)
         {
            if(qst->required_emote[i] != 0)
            {
               for(j = 0; qle->GetMobCount(i) < qst->required_mobcount[i]; ++j)
               {
                  qle->IncrementMobCount( i );
      ...
      case 7://sets area as explored for quest completion
      {
         for( i = 0; i < 4; ++i )
         {
            if( qle->GetExploredAreas(i) != qst->required_triggers[i])
            {
               qle->SetTrigger(i);
               CALL_QUESTSCRIPT_EVENT(qle, OnExploreArea)(qst->required_triggers[i], chr, qle);
               if(qle->CanBeFinished())
               {
                  qle->SendQuestComplete();
               }
      ...
      case 8://marks quest as complete
      {
         qle->SendQuestComplete();
         qle->Complete();
      }break;
      default:
      {
		   RedSystemMessage(m_session, "Enter appropriate assistance type.");
		   return false;
      }break;
   }
   return true;
