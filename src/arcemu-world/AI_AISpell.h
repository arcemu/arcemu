...
struct AISpell_RandomData
{
	...
	//Lua helper methods
	template<int N> ARCEMU_FORCEINLINE uint32 getInterval() const { return interval[N]; }
	template<int N> ARCEMU_FORCEINLINE void setInterval(uint32 nInterval) { interval[N] = nInterval; }
	template<int N> ARCEMU_FORCEINLINE uint16 getFacing() const { return facing[N]; }
	template<int N> ARCEMU_FORCEINLINE void setFacing(uint16 nFacing) { facing[N] = nFacing; }
	template<int N> ARCEMU_FORCEINLINE uint16 getRadius() const { return radius[N]; }
	template<int N> ARCEMU_FORCEINLINE void setRadius(uint16 nRad) { radius[N] = nRad; }
};
...
struct AI_PetSpell
{
	void * bogusvtablePtr;	//structure compatibility w/ AI_Spell, needed since AI_Spell has a virtual method table
