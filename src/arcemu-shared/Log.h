...
class SERVER_DECL oLog : public Singleton< oLog >
{
	public:
		...
		void outErrorSilent(const char* err, ...); // Used for BANNER. No text output.
	...
	private:
		...
		void outFileSilent(FILE* file, char* msg, const char* source = NULL); // Prints text to file without showing it to the user. Used for BANNER.
