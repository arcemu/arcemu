...
int luaGameObject_CastSpell(lua_State * L, GameObject * ptr)
{
	...
	Spell * spp = sSpellMgr.CreateSpell(ptr,dbcSpell.LookupEntry(sp),true, NULL);
...
int luaGameObject_FullCastSpell(lua_State * L, GameObject * ptr)
{
	...
	Spell * nspell = sSpellMgr.CreateSpell(ptr,dbcSpell.LookupEntry(sp),false,NULL);
	SpellCastTargets tar(ptr->GetGUID());
	nspell->prepare(&tar);
	return 0;
}
int luaGameObject_CastSpellOnTarget(lua_State * L, GameObject * ptr)
{
	...
	Spell * nspell = sSpellMgr.CreateSpell(ptr, dbcSpell.LookupEntry(sp), true, NULL);
