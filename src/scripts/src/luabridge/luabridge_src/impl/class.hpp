...
struct classname
{
	...
	{
		classname<T,A>::name_ = name;
	}
};

// Initial type names are unknown
template <typename T, typename A>
const char *classname<T,A>::name_ = classname_unknown;

// Specialization for const types, mapping to same names
template <typename T, typename A>
struct classname <const T, const A>
{
	static const char *name ()
	{
		return classname<T, A>::name_;
...
{
	T ** obj = (T**)checkclass(L, 1, classname<T, A>::name() );
	char buff[55];
	memset(buff, 0, sizeof(char)*55);
	sprintf(buff, "%s (%p)", classname<T, A>::name(), *obj );
...
{
	lua_pushstring(L, classname<T, A>::name() );
	return 1;
}
...
void create_metatable (lua_State *L, const char *name, bool destruct)
{
	...
	lua_pushcfunction(L, (&metaevent_tostring<T,A> ) );
	rawsetfield(L, -2, "__tostring");
	// Set the __gc metamethod to call the class destructor
	if(destruct)
	{
		lua_pushstring(L, name);
		lua_pushcclosure(L, &destructor_dispatch<T, A>, 1);
	...
	lua_pushcfunction(L, (&metaevent_gettypename<T, A> ) );
...
template <typename T, typename A>
class__<T, A>::class__ (lua_State *L_): L(L_)
{
	assert( (classname<T, A>::name() ) != classname_unknown);
}

template <typename T, typename A>
class__<T, A>::class__ (lua_State *L_, const char *name, bool destruct): L(L_)
{
	assert(! (classname<T, A>::is_const() ) );
	classname<T, A>::set_name(name);

	classname<T>::set_name( name);
	classname<A>::set_name( name);

	// Create metatable for this class.  The metatable is stored in the Lua
	// registry, keyed by the given class name.
	create_metatable<T, A>(L, name, destruct);
	...
	create_const_metatable<T, A>(L, name, destruct);
	...
	create_static_table<T, A>(L, name);
	lua_pop(L, 1);
}

template <typename T, typename A>
class__<T, A>::class__ (lua_State *L_, const char *name,
...
int constructor_proxy (lua_State *L)
{
	...
	*block = constructor<A, Params>::apply(args);
...
class__<T, A>& class__<T, A>::constructor ()
{
	// Get a reference to the class's static table
	lua_getglobal(L, (classname<T, A>::name()) );
	// Push the constructor proxy, with the class's metatable as an upvalue
	luaL_getmetatable(L, (classname<T, A>::name()) );
	lua_pushcclosure(L,
		(&constructor_proxy<T, A, typename fnptr<FnPtr>::params > ), 1);
...
template <typename T, typename A>
template <typename FnPtr>
class__<T, A>& class__<T, A>::method (const char *name, FnPtr fp)
{
	assert(fnptr<FnPtr>::mfp);
	std::string metatable_name = classname<T, A>::name();
...
{
	A * obj = *(A**)checkclass(L, 1, lua_tostring(L, lua_upvalueindex(1)));
	U A::* mp = *(U A::**)lua_touserdata(L, lua_upvalueindex(2));
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::property_ro (const char *name, U A::* mp)
{
	luaL_getmetatable(L, (classname<T, A>::name()) );
	std::string constname = std::string("const ") + classname<T, A>::name();
	luaL_getmetatable(L, constname.c_str());
	rawgetfield(L, -2, "__propget");
	rawgetfield(L, -2, "__propget");
	lua_pushstring(L, constname.c_str());
	void *v = lua_newuserdata(L, sizeof(U A::*));
	memcpy(v, &mp, sizeof(U A::*));
	lua_pushcclosure(L, &m_propget_proxy<T, A, U>, 2);
	lua_pushvalue(L, -1);
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::property_ro (const char *name, U (A::*get) () const)
{
	luaL_getmetatable(L, (classname<T, A>::name()) );
	std::string constname = std::string("const ") + classname<T, A>::name();
	luaL_getmetatable(L, constname.c_str());
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::property_rw (const char *name, U A::* mp)
{
	property_ro(name, mp);
	luaL_getmetatable(L, (classname<T, A>::name()) );
	...
	void *v = lua_newuserdata(L, sizeof(U A::*));
	memcpy(v, &mp, sizeof(U A::*));
	lua_pushcclosure(L, &m_propset_proxy<T, A, U>, 2);
	rawsetfield(L, -2, name);
	lua_pop(L, 2);
	return *this;
}
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::property_rw (const char *name,
	U (A::*get) () const, void (A::*set) (U))
{
	property_ro(name, get);
	luaL_getmetatable(L, (classname<T, A>::name()) );
	rawgetfield(L, -1, "__propset");
	lua_pushstring(L, classname<T, A>::name());
	typedef void (A::*FnPtr) (U);
	void *v = lua_newuserdata(L, sizeof(FnPtr));
...
template <typename T, typename A>
template <typename FnPtr>
class__<T, A>& class__<T, A>::static_method (const char *name, FnPtr fp)
{
	assert(!fnptr<FnPtr>::mfp);
	lua_getglobal(L, (classname<T, A>::name()) );
...
template <typename T, typename A>
template <typename U>
class__<T,A>& class__<T,A>::static_property_ro (const char *name, U *data)
{
	lua_getglobal(L, (classname<T,A>::name()) );
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::static_property_ro (const char *name, U (*get) ())
{
	lua_getglobal(L, (classname<T, A>::name()) );
	rawgetfield(L, -1, "__propget");
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::static_property_rw (const char *name, U *data)
{
...
template <typename T, typename A>
template <typename U>
class__<T, A>& class__<T, A>::static_property_rw (const char *name, U (*get) (),
                                            void (*set) (U))
{
