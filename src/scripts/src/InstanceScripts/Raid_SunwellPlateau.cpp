...
class KilJaedenAI : public ArcScriptBossAI
{
	...
	void AIUpdate()
	{
		...
		{
			_unit->CastSpell(Target_Current,47106,true);
			_unit->CastSpell(Target_RandomPlayer,45664,true);
			_unit->CastSpell(Target_RandomPlayer,45642,true);
		}
		if(_unit->GetHealthPct() < 85)
		{

		}
		if(_unit->GetHealthPct() < 55)
		{
		}
