...
public:
    ...
    void OnDied(Unit * mKiller)
    {
		...
		for(Creature::InRangeSet::iterator itr = _unit->GetInRangeSetBegin(); itr != _unit->GetInRangeSetEnd(); itr++)
		{
			Creature *voids = NULL;
			voids = (Creature*)(*itr);
			if(voids != NULL && voids->GetTypeId() == TYPEID_UNIT && voids->GetEntry() == 21101)
				voids->SafeDelete();
		}
