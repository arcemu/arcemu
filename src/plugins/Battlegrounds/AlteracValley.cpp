...
AlteracValley::AVNode::AVNode(AlteracValley* parent, AVNodeTemplate* tmpl, uint32 nodeid)
{
	...
	if(!m_template->m_capturable && m_template->m_initialSpawnId)
	{
		...
		Creature* sp;
		Log.Debug("AlteracValley", "spawning guards at bunker %s of %s (%u)", m_template->m_name, cp->Name, cp->Id);
...
void AlteracValley::AVNode::Spawn()
{
	...
	else
	{
		...
		else
		{
			...
			{
				GameObjectProto* goi = GameObjectProtoStorage.LookupEntry(g->id[m_state]);
