			...
			if( itr != m_resurrectMap.end() )
			{
				for( set<uint32>::iterator it2 = itr->second.begin(); it2 != itr->second.end(); ++it2 )
				{
					...
					if( r_plr != NULL && r_plr->IsDead() )
						sHookInterface.OnRepopRequest(r_plr);
				}
			}
...
bool ArathiBasin::IsInArathiBasin(Player * plr)
{
	if (plr == NULL || !plr->m_bg)
		return false;
	if (plr->m_bg->GetType() != BATTLEGROUND_ARATHI_BASIN)
		return false;
	return true;
}

void ArathiBasin::OnPlayerDeath(Player * plr)
{
	if (!IsInArathiBasin(plr))
		return;
...
void ArathiBasin::OnAreaTrigger(Player * plr, uint32 id)
{
	if (!IsInArathiBasin(plr))
		return;

	ArathiBasin * bg = static_cast<ArathiBasin *>(plr->m_bg);
	...
	if(bg->GetBuff(x) && bg->GetBuff(x)->IsInWorld())
	{
		// apply the spell
		spellid = bg->GetBuff(x)->GetInfo()->sound3;
		bg->GetBuff(x)->RemoveFromWorld(false);

		// respawn it in buffrespawntime
		sEventMgr.AddEvent(bg, &ArathiBasin::SpawnBuff, x, EVENT_AB_RESPAWN_BUFF, AB_BUFF_RESPAWN_TIME, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
...
bool ArathiBasin::OnSpellCast(Player * pCaster, SpellEntry * pSpell)
{
	if (!IsInArathiBasin(pCaster))
		return false;
	...
	if (pSpell->Id == 21651)
	{
		...
		pCaster->RemoveAllAuraByNameHash(SPELL_HASH_DIVINE_PROTECTION);
...
bool ArathiBasin::OnRepopRequest(Player * plr)
{
	if (!IsInArathiBasin(plr))
		return false;
	...
	for(uint32 i = 0; i < AB_NUM_CONTROL_POINTS; ++i)
	{
		if (bg->IsBaseOwnedBy(i, plr->GetTeam()))
		{
...
void ArathiBasin::OnHonorKill(Player * plr)
{
	if (!IsInArathiBasin(plr))
		return;
...
void ArathiBasin::OnPlayerKill(Player * plr, Player* pVictim)
{
	if (!IsInArathiBasin(plr))
		return;
...
void ArathiBasin::OnDestoryGameObject(GameObject * gobj)
{
	//if (!IsInArthathiBasin(plr))
	ASSERT(gobj != NULL);
	if (!gobj->m_battleground && gobj->m_battleground->GetType() != BATTLEGROUND_ARATHI_BASIN)
		return;

	static_cast<ArathiBasin *>(gobj->m_battleground)->RemoveControlPoint(gobj);
}


void SetupArathiBasin(ScriptMgr * mgr)
{
	uint32 nameId = 40;

	// Register Battleground, constants here are as in 3.0.3
	BattlegroundManager.RegisterBattleground(BATTLEGROUND_ARATHI_BASIN, MAP_ARATHI_BASIN,
		nameId, 20, 10, 15, &ArathiBasin::Create);

	// Battlemaster Interaction Handlers
