...
void EyeOfTheStorm::GivePlayerFlag(Player * plr)
{
...
void EyeOfTheStorm::HookOnSpellCast(Player * p_caster, uint32 spellId)
{
	sLog.outDebug("*** Start HookOnSpellCast");
	...
	sLog.outDebug("*** End HookOnSpellCast");
}

void EyeOfTheStorm::HookOnDestoryGameObject(GameObject * gameobject)
{
}
...
bool EyeOfTheStorm::IsInEyeOfTheStorm(Player * plr)
{
	if (plr == NULL || !plr->m_bg)
		return false;
	if (plr->m_bg->GetType() != BATTLEGROUND_EYE_OF_THE_STORM)
		return false;
	return true;
}

void EyeOfTheStorm::OnPlayerDeath(Player * plr)
{
	if (!IsInEyeOfTheStorm(plr))
		return;

	EyeOfTheStorm * bg = static_cast<EyeOfTheStorm *>(plr->m_bg);

	plr->m_bgScore.Deaths++;

	bg->PlayerDropFlag(plr);
}

void EyeOfTheStorm::OnFlagDropPickup(Player * plr, GameObject * obj)
{
	if (!IsInEyeOfTheStorm(plr))
		return;

	EyeOfTheStorm * bg = static_cast<EyeOfTheStorm *>(plr->m_bg);
	GameObject * droppedFlag = bg->m_dropFlag;
	...
	bg->GivePlayerFlag(plr);

}

void EyeOfTheStorm::OnMount(Player * plr)
{
	if (!IsInEyeOfTheStorm(plr))
		return;
...
void EyeOfTheStorm::OnAreaTrigger(Player * plr, uint32 id)
{
	if (!IsInEyeOfTheStorm(plr))
		return;
	...
	if (bonusid > -1)
	{
		...
		GameObject * buff = bg->GetBuff(x);
		if (buff && buff->IsInWorld())
		{
			spellid = buff->GetInfo()->sound3;
			...
			buff->Despawn(0, EOTS_BUFF_RESPAWN_TIME);
		}
	}
	...
	if (!bg->HasStarted())
	{
		Anticheat_Log->writefromsession(plr->GetSession(), "%s tryed to hook the flag in eye of the storm before battleground (ID %u) started.", plr->GetName(), bg->GetId());
	...
	if (bg->m_CPStatus[tid] != val )
		return;			// not captured by our team

	for(i = 0; i < EOTS_TOWER_COUNT; ++i)
	{
		if (bg->m_CPStatus[i] == val)
			towers++;
	}
	...
	*/
	...
	bg->SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + team, 0, msgs[team] );
	bg->GivePoints( team, points[towers] );
	bg->PushFlagToStand();
...
bool EyeOfTheStorm::OnRepopRequest(Player * plr)
{
	if (!IsInEyeOfTheStorm(plr))
		return false;
...
void EyeOfTheStorm::OnFlagDrop(Player * plr)
{
	if (!IsInEyeOfTheStorm(plr))
		return;
	...
	if( bg->m_flagHolder != plr->GetLowGUID() )
		return;

	plr->RemoveAura( EOTS_NETHERWING_FLAG_SPELL );

	bg->m_dropFlag->SetPosition( plr->GetPosition() );
	bg->m_dropFlag->PushToWorld( bg->m_mapMgr );
	bg->m_flagHolder = 0;

	bg->SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + plr->GetTeam(), plr->GetGUID(), "$N has dropped the flag!" );

	sEventMgr.AddEvent( bg, &EyeOfTheStorm::EventResetFlag, EVENT_EOTS_RESET_FLAG, 60000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT );
}

void EyeOfTheStorm::OnPlayerKill(Player * plr, Player * pVictim)
{
	if (!IsInEyeOfTheStorm(plr))
		return;
...
void EyeOfTheStorm::OnHonorKill(Player * plr)
{
	if (!IsInEyeOfTheStorm(plr))
		return;
...
void SetupEyeOfTheStorm(ScriptMgr * mgr)
{
	uint32 nameId = 44;

	// Register the battleground
	BattlegroundManager.RegisterBattleground(BATTLEGROUND_EYE_OF_THE_STORM, MAP_EYE_OF_THE_STORM,
		nameId, 61, 15, 10, &EyeOfTheStorm::Create);

	// Register Battlemaster Interaction Handler
