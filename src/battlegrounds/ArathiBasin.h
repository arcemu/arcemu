...
#ifndef HEADER_ARATHI_BASIN
#define HEADER_ARATHI_BASIN
...
public:
	...
	ARCEMU_INLINE GameObject * GetBuff(uint32 buff)
	{
		ASSERT(buff <= 0 && buff < AB_NUM_BUFFS);
		return m_buffs[buff];
	}

	ARCEMU_INLINE bool IsBaseOwnedBy(uint32 i, uint32 team)
	{
		return (m_basesOwnedBy[i] == team);
	}

	ARCEMU_INLINE void RemoveControlPoint(GameObject * gobj)
	{
		if( gobj->bannerslot >= 0 && m_controlPoints[gobj->bannerslot] == gobj )
			m_controlPoints[gobj->bannerslot] = NULL;

		if( gobj->bannerauraslot >= 0 && m_controlPointAuras[gobj->bannerauraslot] == gobj )
			m_controlPointAuras[gobj->bannerauraslot] = NULL;
	}
	...
	/* Event Handlers */
	static bool IsInArathiBasin(Player * plr);
	static void OnPlayerDeath(Player * plr);
	static void OnAreaTrigger(Player * plr, uint32 id);
	static bool OnSpellCast(Player * pCaster, SpellEntry * pSpell);
	...
	static void OnDestoryGameObject(GameObject * gobj);
