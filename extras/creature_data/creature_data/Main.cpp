...
void main()
{
	...
	for (auto itr = displayInfo.begin(); itr != displayInfo.end(); ++itr)
	{
		...
		M2Header hdr;
		modelf.read(&hdr, sizeof(M2Header));

		printf(" %u attachments %u bone lookups %u bones\n", hdr.nAttachments, hdr.nBoneLookupTable, hdr.nBones);

		M2Attachment* attachments = (M2Attachment*)malloc(hdr.nAttachments * sizeof(M2Attachment));
		modelf.seek(hdr.ofsAttachments);
		modelf.read(attachments, hdr.nAttachments * sizeof(M2Attachment));

 		uint16* bonelookups = (uint16*)malloc(hdr.nBoneLookupTable * sizeof(uint16));
 		modelf.seek(hdr.ofsBoneLookupTable);
		modelf.read(bonelookups, hdr.nBoneLookupTable * sizeof(uint16));

		M2Bone* bones = (M2Bone*)malloc(hdr.nBones * sizeof(M2Bone));
		modelf.seek(hdr.ofsBones);
		modelf.read(bones, hdr.nBones * sizeof(M2Bone));

		//try and get the bone
		for (auto i = 0; i < hdr.nAttachments; ++i)
		{
			if (attachments[i].bone > hdr.nBoneLookupTable)
			{
				printf("Attachment %u requests bonelookup %u (too large, bonelookup table is only %u entries)\n", i, attachments[i].bone, hdr.nBoneLookupTable);
			}
			uint16 boneindex = bonelookups[attachments[i].bone];
			if (boneindex > hdr.nBones)
			{
				printf("Attachment %u requests bone %u (too large, bone table is only %u entries)\n", i, boneindex, hdr.nBones);
				continue;
			}
...
// 			realpos[1] = -tmp;
		...
		float boundlow[3];
		boundlow[0] = hdr.boundingbox1[0] * modelscale;
		boundlow[1] = hdr.boundingbox1[1] * modelscale;
		boundlow[2] = hdr.boundingbox1[2] * modelscale;
		float boundhigh[3];
		boundhigh[0] = hdr.boundingbox2[0] * modelscale;
		boundhigh[1] = hdr.boundingbox2[1] * modelscale;
		boundhigh[2] = hdr.boundingbox2[2] * modelscale;
