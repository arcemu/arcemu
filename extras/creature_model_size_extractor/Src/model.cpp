...
//don't shoot me because i failed a few math classes. I have no idea what indices and normals are :(. 
//I'm guessing vertices is a good aproximation for size
float Model::GetModelCanReachRadius()
{
	float ret = 0;
    int nVertices = header.nVertices;
    if(nVertices >0) 
    {
		...
		minx = miny = minz = (float)0x0FFFFFFF;
		maxx = maxy = maxz = (float)-0x0FFFFFFF;
        for(int vpos=0; vpos <nVertices; ++vpos) 
        {
			if( vertices[vpos].x < minx )
				minx = vertices[vpos].x;
			if( vertices[vpos].x > maxx )
				maxx = vertices[vpos].x;
			if( vertices[vpos].y < miny )
				miny = vertices[vpos].y;
			if( vertices[vpos].y > maxy )
				maxy = vertices[vpos].y;
			if( vertices[vpos].z < minz )
				minz = vertices[vpos].z;
			if( vertices[vpos].z > maxz )
				maxz = vertices[vpos].z;
        }
		float dx = abs( minx - maxx );
//		float dy = abs( miny - maxy );
		float dz = abs( minz - maxz );
		// models are rendered in sleeping position -> we swap Y with Z to make them "stand"
		if( dx > dz )
			ret = dx / 2; //size is double radious
		else
			ret = dz / 2; //size is double radious
	}
    return ret;
}
