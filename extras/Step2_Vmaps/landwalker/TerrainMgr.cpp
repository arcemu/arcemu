...
TerrainMgr::TerrainMgr(std::string MapPath, uint32 MapId, bool Instanced) : mapPath(MapPath), mapId(MapId), Instance(Instanced)
{
...
bool TerrainMgr::LoadTerrainHeader()
{
	...
	if(FileDescriptor == 0)
	{
//		printf("Could not load map file header for %s.\n", File);
		return false;
	}
	...
	if(ftell(FileDescriptor) == 1048576)
	{
		/* file with no data */
		fclose(FileDescriptor);
//		printf("File is empty, there is nothing to load.\n");
		FileDescriptor=NULL;
		return false;
	}
	...
	{
		fclose(FileDescriptor);
		printf("Header is wrong, can't load\n");
		FileDescriptor=NULL;
		return false;
	}
	...
	hMappedFile = CreateFile(File, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_ARCHIVE, NULL);
	if(hMappedFile == INVALID_HANDLE_VALUE)
		return false;

	hMap = CreateFileMapping(hMappedFile, NULL, PAGE_READWRITE, 0, 0, NULL);
	...
	m_Memory = (uint8*)MapViewOfFile(hMap, FILE_MAP_READ | FILE_MAP_WRITE, 0, TERRAIN_HEADER_SIZE, mFileSize-TERRAIN_HEADER_SIZE);
	if(m_Memory==NULL)
	{
		CloseHandle(hMap);
		CloseHandle(hMappedFile);
...
bool TerrainMgr::UnloadCellInformation(uint32 x, uint32 y)
{
	...
	printf("Unloaded cell information for cell [%u][%u] in %ums.", x, y, GetTickCount() - Start);
	// Success
	return true;
#endif
}
...
bool TerrainMgr::Save_map_to_file(char *new_filename)
{
	//open up the file
	FileDescriptor = fopen(new_filename, "wb");

	if( FileDescriptor )
	{
		//write header
		fwrite( CellOffsets,TERRAIN_HEADER_SIZE,1,FileDescriptor );
		//now write back the cells as we read them. 
		//in case this is buget then we should regenerate offsets maybe. Until then trust the man :D
        for(uint32 x = 0; x < 512; ++x)
            for(uint32 y = 0; y < 512; ++y)
				if( CellInformation[x][y] )
					fwrite( CellInformation[x][y],sizeof( CellTerrainInformation ),1,FileDescriptor );
